...
#include "GEOADVPPositionTable.h"

namespace inet {

std::vector<L3Address> GEOADVPPositionTable::getAddresses() const
{
...
bool GEOADVPPositionTable::hasPosition(const L3Address& address) const
{
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
    return it != addressToPositionMap.end();
}

Coord GEOADVPPositionTable::getPosition(const L3Address& address) const
{
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
...
void GEOADVPPositionTable::setPosition(const L3Address& address, const Coord& coord)
{
    ASSERT(!address.isUnspecified());
    addressToPositionMap[address] = AddressToPositionMapValue(simTime(), coord);
}

void GEOADVPPositionTable::removePosition(const L3Address& address)
{
    auto it = addressToPositionMap.find(address);
    addressToPositionMap.erase(it);
}

void GEOADVPPositionTable::removeOldPositions(simtime_t timestamp)
{
...
void GEOADVPPositionTable::clear()
{
    addressToPositionMap.clear();
}

simtime_t GEOADVPPositionTable::getOldestPosition() const
{
...
std::ostream& operator << (std::ostream& o, const GEOADVPPositionTable& t)
{
    o << "{ ";
