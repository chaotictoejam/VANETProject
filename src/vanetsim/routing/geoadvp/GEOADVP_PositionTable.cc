...
std::vector<L3Address> GEOADVP_PositionTable::getAddresses() const
{
...
bool GEOADVP_PositionTable::hasPosition(const L3Address& address) const
{
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
    return it != addressToPositionMap.end();
}

Coord GEOADVP_PositionTable::getPosition(const L3Address& address) const
{
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
...
void GEOADVP_PositionTable::setPosition(const L3Address& address, const Coord& coord)
{
    ASSERT(!address.isUnspecified());
    addressToPositionMap[address] = AddressToPositionMapValue(simTime(), coord);
}

void GEOADVP_PositionTable::removePosition(const L3Address& address)
{
    auto it = addressToPositionMap.find(address);
    addressToPositionMap.erase(it);
}

void GEOADVP_PositionTable::removeOldPositions(simtime_t timestamp)
{
...
void GEOADVP_PositionTable::clear()
{
    addressToPositionMap.clear();
}

simtime_t GEOADVP_PositionTable::getOldestPosition() const
{
...
std::ostream& operator << (std::ostream& o, const GEOADVP_PositionTable& t)
{
    o << "{ ";
