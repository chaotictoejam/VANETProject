...
#ifndef __INET_AODVWRROUTING_H
#define __INET_AODVWRROUTING_H
...
/*
 * This class implements AODVWR routing protocol and Netfilter hooks
 * in the IP-layer required by this protocol.
 */

class INET_API AODVWRRouting : public cSimpleModule, public ILifecycle, public INetfilter::IHook, public cListener
{
  protected:
    /*
    ...
    // AODVWR parameters: the following parameters are configurable, see the NED file for more info.
    ...
    std::map<L3Address, WaitForRREPWR *> waitForRREPTimers;    // timeout for Route Replies
  ...
  protected:
    ...
    void updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, double twr, double expirationTime);
    IRoute *createRoute(const L3Address& destAddr, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, double twr, double expirationTime);
    bool updateValidRouteExpirationTime(const L3Address& destAddr, double expirationTime);
    void scheduleExpungeRoutes();
    void expungeRoutes();

    /* Control packet creators */
    AODVWRRREPACK *createRREPACK();
    AODVWRRREP *createHelloMessage();
    AODVWRRREQ *createRREQ(const L3Address& destAddr);
    AODVWRRREP *createRREP(AODVWRRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& sourceAddr);
    AODVWRRREP *createGratuitousRREP(AODVWRRREQ *rreq, IRoute *originatorRoute);
    AODVWRRERR *createRERR(const std::vector<UnreachableAODVWRNode>& unreachableNodes);

    /* Control Packet handlers */
    void handleRREP(AODVWRRREP *rrep, const L3Address& sourceAddr);
    void handleRREQ(AODVWRRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive);
    void handleRERR(AODVWRRERR *rerr, const L3Address& sourceAddr);
    void handleHelloMessage(AODVWRRREP *helloMessage);
    void handleRREPACK(AODVWRRREPACK *rrepACK, const L3Address& neighborAddr);

    /* Control Packet sender methods */
    void sendRREQ(AODVWRRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive);
    void sendRREPACK(AODVWRRREPACK *rrepACK, const L3Address& destAddr);
    void sendRREP(AODVWRRREP *rrep, const L3Address& destAddr, unsigned int timeToLive);
    void sendGRREP(AODVWRRREP *grrep, const L3Address& destAddr, unsigned int timeToLive);

    /* Control Packet forwarders */
    void forwardRREP(AODVWRRREP *rrep, const L3Address& destAddr, unsigned int timeToLive);
    void forwardRREQ(AODVWRRREQ *rreq, unsigned int timeToLive);

    /* Self message handlers */
    void handleRREPACKTimer();
    void handleBlackListTimer();
    void sendHelloMessagesIfNeeded();
    void handleWaitForRREPWR(WaitForRREPWR *rrepTimer);
   ...
   L3Address getSelfIPAddress() const;
    void sendAODVWRPacket(AODVWRControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay);
    void clearState();
  ...
  public:
    AODVWRRouting();
    virtual ~AODVWRRouting();
};

} // namespace inet

#endif    // ifndef AODVWRROUTING_H_
