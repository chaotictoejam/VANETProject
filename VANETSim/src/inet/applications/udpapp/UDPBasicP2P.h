 ...
 */
class INET_API UDPBasicP2P : public cSimpleModule
{
public:
    enum  modoP2P
    {
...
private:
  ...
  protected:
    ...
    bool areDiff(const SegmentList &List1, const SegmentList &List2);
    virtual int numInitStages() const {return 7;}
    virtual void initialize(int stage);
    virtual void finish();

    bool processMyTimer(cMessage *);
    // chooses random destination address
    void generateRequestNew();
    void generateRequestSub();
    bool processPacket(cPacket *pk);
    void processRequest(cPacket *pkt);
    void sendRateLimit(UDPBasicPacketP2P *pkt, const uint64_t &,double rate);
    void sendQueue();
    ...
    virtual std::vector<UDPBasicP2P::InfoData> selectBestList(const std::vector<uint64_t> &address, const std::vector<SegmentList> &List);
    virtual std::vector<UDPBasicP2P::InfoData>  chooseDestAddrList();
  ...
  public:
   ...
   protected:
     ...
     //statistics:
     // chooses random destination address
     virtual L3Address chooseDestAddrBurst();
