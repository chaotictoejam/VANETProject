...
void NS_CLASS initialize(int stage)
{
    ...
    if (stage==4)
    {
        ...
        for (int i = 0; i <getNumInterfaces(); i++)
        {
            ...
            if (!isInMacLayer())
            {
                DEV_NR(i).netmask.s_addr =
                    VanetAddress(getInterfaceEntry(i)->ipv4Data()->getIPAddress().getNetworkMask());
                DEV_NR(i).ipaddr.s_addr =
                        VanetAddress(getInterfaceEntry(i)->ipv4Data()->getIPAddress());
            }
            else
            {
                DEV_NR(i).netmask.s_addr = VanetAddress(MACAddress::BROADCAST_ADDRESS);
                DEV_NR(i).ipaddr.s_addr = VanetAddress(getInterfaceEntry(i)->getMacAddress());
        ...
        for (int i=0; i < getNumWlanInterfaces(); i++)
        {
            ...
            DEV_NR(getWlanInterfaceIndex(i)).broadcast.s_addr = VanetAddress(IPv4Address(AODVVANET_BROADCAST));
        }
        ...
        INIT_AVLIST_HEAD(&rreq_records);
        lista_ptr=&rreq_blacklist;
        INIT_AVLIST_HEAD(&rreq_blacklist);
        lista_ptr=&seekhead;
        INIT_AVLIST_HEAD(&seekhead);
...
/* Destructor for the AODV-VANET routing agent */
NS_CLASS ~AODVVANET()
{
...
#else
    avlist_t *tmp = NULL, *pos = NULL;
    for (int i = 0; i < RT_TABLESIZE; i++)
    {
        avlist_foreach_safe(pos, tmp, &rt_tbl.tbl[i])
        {
            rt_table_t *rt = (rt_table_t *) pos;
            avlist_detach(&rt->l);
            precursor_avlist_destroy(rt);
            free(rt);
        }
    }
#endif
#ifndef AODVVANET_USE_STL
    while (!avlist_empty(&rreq_records))
    {
        pos = avlist_first(&rreq_records);
        avlist_detach(pos);
        if (pos) free(pos);
    }

    while (!avlist_empty(&rreq_blacklist))
    {
        pos = avlist_first(&rreq_blacklist);
        avlist_detach(pos);
        if (pos) free(pos);
    }

    while (!avlist_empty(&seekhead))
    {
        pos = avlist_first(&seekhead);
        avlist_detach(pos);
        if (pos) free(pos);
    }
#else
...
void NS_CLASS packetFailed(IPv4Datagram *dgram)
{
    ...
    src_addr.s_addr = VanetAddress(dgram->getSrcAddress());
    dest_addr.s_addr = VanetAddress(dgram->getDestAddress());
    ...
    if (dgram->getDestAddress().getInt() == IP_BROADCAST ||
            dgram->getDestAddress().getInt() == AODVVANET_BROADCAST)
    {
...
void NS_CLASS packetFailedMac(Ieee80211DataFrame *dgram)
{
    ...
    src_addr.s_addr = VanetAddress(dgram->getAddress3());
    dest_addr.s_addr = VanetAddress(dgram->getAddress4());
    if (avseek_list_find(dest_addr))
    {
        DEBUG(LOG_DEBUG, 0, "Ongoing route discovery, buffering packet...");
    ...
    next_hop.s_addr = VanetAddress(dgram->getReceiverAddress());
...
void NS_CLASS handleMessage (cMessage *msg)
{
    ...
    if (msg->isSelfMessage() && dynamic_cast<AODVVANET_msg*> (msg))
    {
        ...
        if (msg)
            aodvvanet_socket_send((AODVVANET_msg *) msg, delayInfo->dst , delayInfo->len, delayInfo->ttl, delayInfo->dev,0);
        delete delayInfo;
        return;
    }
    ...
    if (dynamic_cast<ControlVanetRouting *>(msg))
    {
        ControlVanetRouting * control =  check_and_cast <ControlVanetRouting *> (msg);
        if (control->getOptionCode()== VANET_ROUTE_NOROUTE)
        {
            if (isInMacLayer())
            {
                ...
                else
                {
                    if (!addressIsForUs(control->getSrcAddress()))
                    {
                        ...
                        if (rev_rt && rev_rt->state == VALID)
                            ...
                            rerr_dest.s_addr = VanetAddress(IPv4Address(AODVVANET_BROADCAST));

                        aodvvanet_socket_send((AODVVANET_msg *) rerr, rerr_dest,AODVVANETRERR_CALC_SIZE(rerr), 1, &DEV_IFINDEX(NS_IFINDEX));
                    }
                }
            }
            else
            {
                ...
                if (ctrl)
                {
                    if (dynamic_cast<Ieee802Ctrl*> (ctrl))
                    {
                        ...
                        VanetAddress address(ieeectrl->getDest());
        ...
        else if (control->getOptionCode()== VANET_ROUTE_UPDATE)
        {
    ...
    else if (dynamic_cast<UDPPacket *>(msg) || dynamic_cast<AODVVANET_msg  *>(msg))
    {
        udpPacket = NULL;
        if (!isInMacLayer())
        {
...
/* Starts the AODV-VANET routing agent */
int NS_CLASS startAODVVANETAgent()
{
...
void NS_CLASS recvAODVVANETPacket(cMessage * msg)
{
    struct in_addr src, dst;
    int ttl;
    int interfaceId;

    AODVVANET_msg *aodvvanet_msg = check_and_cast<AODVVANET_msg *> (msg);
    int len = aodvvanet_msg->getByteLength();
    int ifIndex = NS_IFINDEX;

    ttl =  aodvvanet_msg->ttl-1;
    if (!isInMacLayer())
    {
        ...
        src.s_addr = VanetAddress(srcAddr);
        dst.s_addr =  VanetAddress(destAddr);
    ...
    else
    {
        ...
        src.s_addr = VanetAddress(ctrl->getSrc());
        dst.s_addr =  VanetAddress(ctrl->getDest());
    }
    ...
    aodvvanet_socket_process_packet(aodvvanet_msg, len, src, dst, ttl, ifIndex);
    delete   aodvvanet_msg;
}


void NS_CLASS processMacPacket(cPacket * p, const VanetAddress &dest, const VanetAddress &src, int ifindex)
{
            ...
            (fwd_rt->hcnt == 1 && (fwd_rt->flags & RT_UNIDIR)))
    {
        ...
        // Else we must send a AODVVANETRERR message to the source if
        ...
        else
        {
            ...
            if (fwd_rt)
            {
                ...
                rerr = rerr_create(0, dest_addr, 0);
            DEBUG(LOG_DEBUG, 0, "Sending AODVVANETRERR to prev hop %s for unknown dest %s",
                    ip_to_str(src_addr), ip_to_str(dest_addr));

                /* Unicast the AODVVANETRERR to the source of the data transmission
            ...
            if (rev_rt && rev_rt->state == VALID)
                ...
                rerr_dest.s_addr = VanetAddress(IPv4Address(AODVVANET_BROADCAST));
            aodvvanet_socket_send((AODVVANET_msg *) rerr, rerr_dest,AODVVANETRERR_CALC_SIZE(rerr),
...
void NS_CLASS processPacket(IPv4Datagram * p,unsigned int ifindex)
{
    ...
    src_addr.s_addr = VanetAddress(p->getSrcAddress());
    dest_addr.s_addr = VanetAddress(p->getDestAddress());

    InterfaceEntry *   ie;

    if (!p->getSrcAddress().isUnspecified())
    {
        isLocal = isLocalAddress(VanetAddress(p->getSrcAddress()));
    }
    ...
    if (isMcast || dest_addr.s_addr == VanetAddress(IPv4Address(AODVVANET_BROADCAST)))
    {
        send(p,"to_ip");
        return;
    }
...
void NS_CLASS processFullPromiscuous(const cObject *details)
{
    if (dynamic_cast<Ieee80211TwoAddressFrame *>(const_cast<cPolymorphic*> (details)))
    {
        ...
        VanetAddress sender(frame->getTransmitterAddress());
            ...
            if (meshFrame->getSubType() == ROUTING)
            {
                ...
                if (pktAux2 && dynamic_cast<RREP *> (pktAux2) && checkRrep)
                {
                    ...
                    if (it == rrepProc.end())
                    {
                        ...
                        rproc.next = VanetAddress(meshFrame->getReceiverAddress());
                    }
                    else if (it->second.dest_seqno < rrep->dest_seqno)
                    {
                        ...
                        it->second.next = VanetAddress(meshFrame->getReceiverAddress());
                    }
                    else if (it->second.dest_seqno == rrep->dest_seqno)
                    {
                        // equal seq num check
...
uint32_t NS_CLASS getRoute(const VanetAddress &dest,std::vector<VanetAddress> &add)
{
    return 0;
}


bool  NS_CLASS getNextHop(const VanetAddress &dest,VanetAddress &add, int &iface,double &cost)
{
...
void NS_CLASS setRefreshRoute(const VanetAddress &destination, const VanetAddress & nextHop,bool isReverse)
{
...
{
    AODVVANET_msg *re = dynamic_cast <AODVVANET_msg *>(msg);
    if (re)
        return true;
    return false;
}
...
#ifdef AODVVANET_USE_STL_RT
bool  NS_CLASS setRoute(const VanetAddress &dest,const VanetAddress &add, const int &ifaceIndex,const int &hops,const VanetAddress &mask)
{
    Enter_Method_Silent();
    struct in_addr destAddr;
    ...
    if (fwd_rt)
    {
        if (delEntry)
        {
            AODVVANETRERR* rerr = rerr_create(0, destAddr, 0);
        ...
        VanetAddress dest = fwd_rt->dest_addr.s_addr;
    ...
    if (ifaceIndex>=getNumInterfaces())
        status = false;
    VanetRoutingBase::setRoute(dest,add,ifaceIndex,hops,mask);
...
bool  NS_CLASS setRoute(const VanetAddress &dest,const VanetAddress &add, const char  *ifaceName,const int &hops,const VanetAddress &mask)
{
    Enter_Method_Silent();
    struct in_addr destAddr;
    struct in_addr nextAddr;
    ...
    VanetRoutingBase::setRoute(dest,add,index,hops,mask);
...
bool  NS_CLASS setRoute(const VanetAddress &dest,const VanetAddress &add, const int &ifaceIndex,const int &hops,const VanetAddress &mask)
{
    ...
    nextAddr.s_addr = add;
    bool status=true;
    bool delEntry = (add == (VanetAddress)0);
    ...
    if (fwd_rt)
    {
        ...
        avlist_detach(&fwd_rt->l);
        precursor_avlist_destroy(fwd_rt);
        if (fwd_rt->state == VALID || fwd_rt->state == IMMORTAL)
            rt_tbl.num_active--;
        timer_remove(&fwd_rt->rt_timer);
        timer_remove(&fwd_rt->hello_timer);
...
void NS_CLASS actualizeTablesWithCollaborative(const VanetAddress &dest)
{
    ...
    if (getCollaborativeProtocol()->getNextHop(dest,next_hop.s_addr,iface,cost))
    {
        ...
        std::map<VanetAddress,u_int32_t *>::iterator it =  mapSeqNum.find(dest);
        if (it == mapSeqNum.end())
