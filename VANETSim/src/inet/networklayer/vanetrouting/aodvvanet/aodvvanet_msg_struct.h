...
// 8 for IPv4
//#define AODVVANETRERR_UDEST_SIZE 4+VanetAddressSizeUsed

#ifdef AODVVANETRERR
#undef AODVVANETRERR
#endif

struct AODVVANETRERR : public AODVVANET_msg
{
    unsigned short res1;
    unsigned short n;
    unsigned short res2;
    u_int8_t dest_count;
    AODVVANETRERR_udest *   _udest;
    explicit AODVVANETRERR(const char *name="AODVVANETRERRAodvMsg");
    ~AODVVANETRERR ();
    AODVVANETRERR (const AODVVANETRERR &m);
    ...
    void addUdest(const VanetAddress &,unsigned int);
    void clearUdest();
    AODVVANETRERR_udest *getUdest(int);
    AODVVANETRERR_udest& getUdestRef(int i) {return *getUdest(i);}
    AODVVANETRERR &  operator= (const AODVVANETRERR &m);
    virtual AODVVANETRERR *dup() const {return new AODVVANETRERR(*this);}
  private:
    void copy(const AODVVANETRERR& other);
};

#define AODVVANETRERR_UDEST_FIRST(rerr) (rerr->getUdest(0))
#define AODVVANETRERR_UDEST_NEXT(udest) ((AODVVANETRERR_udest *)((char *)udest + sizeof(AODVVANETRERR_udest)))
// 12 for IPv4
