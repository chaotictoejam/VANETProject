...
void IPv4RoutingTable::addRule(bool output,IPv4RouteRule *entry)
{
...
const IPv4RouteRule * IPv4RoutingTable::getRule(bool output,int index) const
{
    if (output)
    {
...
int IPv4RoutingTable::getNumRules(bool output)
{
    if (output)
...
const IPv4RouteRule * IPv4RoutingTable::findRule(bool output,int prot,int sPort,const IPv4Address &srcAddr,int dPort,const IPv4Address &destAddr,const InterfaceEntry *iface) const
{
    ...
    while (it!=endIt)
    {
       ...
       if ((iface!=nullptr) && (e->getInterface()!=nullptr) && (iface!=e->getInterface()))
       {
           it++;
           continue;
       }
