...
void ManetRoutingBase::registerRoutingModule()
{
    ...
    locator = dynamic_cast<ILocator *>(getContainingNode(this)->getSubmodule("locator"));
    if (locator)
    {
        ...
        hostModule->subscribe(NF_LOCATOR_ASSOC, this);
        hostModule->subscribe(NF_LOCATOR_DISASSOC, this);
    }
#endif
...
// If found interface We use the address of interface
        ...
        if ((add == IPv4Address::ALLONES_ADDRESS || add == IPv4Address::LL_MANET_ROUTERS) && ie == nullptr)
        {
...
void ManetRoutingBase::omnet_clean_rte()
{
    ...
    {
        entry = inet_rt->getRoute(i);
        if (strstr(entry->getInterface()->getName(), "wlan")!=nullptr)
        {
            inet_rt->deleteRoute(entry);
        }
    }
    ...
    else if (signalID == NF_LINK_BREAK)
    {
        ...
        else if (dynamic_cast<XMacPkt *>(const_cast<cObject*>(obj)))
        {
            ...
            if (!mac_layer_ && pktAux != nullptr)
            {
                cPacket *pkt = pktAux->dup();
                ControlInfoBreakLink *add = new ControlInfoBreakLink;
                add->setDest(frame->getDestinationAddress(0));
                pkt->setControlInfo(add);
                processLinkBreak(pkt);
                delete pkt;
            }
...
#ifdef WITH_80211MESH
    else if(signalID == NF_LOCATOR_ASSOC)
        processLocatorAssoc(obj);
    else if(signalID == NF_LOCATOR_DISASSOC)
        processLocatorDisAssoc(obj);
#endif
}
...
bool ManetRoutingBase::getAp(const L3Address &destination, L3Address& accesPointAddr) const
{
    ...
    if (isInMacLayer())
    {
        MACAddress macAddr = locator->getLocatorMacToMac(destination.toMAC());
        if (macAddr.isUnspecified())
            return false;
        accesPointAddr = L3Address(macAddr);
    }
    else
    {
        IPv4Address ipAddr = locator->getLocatorIpToIp(destination.toIPv4());
        if (ipAddr.isUnspecified())
            return false;
        accesPointAddr = L3Address(ipAddr);
    }
    return true;
#else
    return false;
