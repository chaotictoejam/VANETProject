...
#include "world/traci/bitcarTraCIScenarioManagerLaunchd.h"
...
Define_Module(bitcarTraCIScenarioManagerLaunchd);

bitcarTraCIScenarioManagerLaunchd::~bitcarTraCIScenarioManagerLaunchd()
{
}


void bitcarTraCIScenarioManagerLaunchd::initialize(int stage)
{
    if (stage != 1) {
        bitcarTraCIScenarioManager::initialize(stage);
        return;
    }
...
void bitcarTraCIScenarioManagerLaunchd::finish()
{
    bitcarTraCIScenarioManager::finish();
}

void bitcarTraCIScenarioManagerLaunchd::init_traci() {
    {
        ...
        MYDEBUG << "bitcarTraCI launchd reports version \"" << serverVersion << "\"" << endl;
    }
    ...
    sendbitcarTraCIMessage(makebitcarTraCICommand(CMD_FILE_SEND, buf));
    ...
    if (result != RTYPE_OK) {
        EV << "Warning: Received non-OK response from bitcarTraCI server to command " << CMD_FILE_SEND << ":" << description.c_str() << std::endl;
    }

    bitcarTraCIScenarioManager::init_traci();
}
