...
Define_Module(BITCARRouting);

void BITCARRouting::initialize(int stage)
{
    if (stage == 0) {
...
void BITCARRouting::handleMessage(cMessage *msg)
{
    if (!isOperational) {
    ...
    if (msg->isSelfMessage()) {
        ...
        BITCARControlPacket *ctrlPacket = check_and_cast<BITCARControlPacket *>(udpPacket->decapsulate());
        ...
        switch (ctrlPacket->getPacketType()) {
            case AODVVANETRREQ:
                handleAODVVANETRREQ(check_and_cast<BITCARAODVVANETRREQ *>(ctrlPacket), sourceAddr, arrivalPacketTTL);
                break;

            case RREP:
                handleRREP(check_and_cast<BITCARRREP *>(ctrlPacket), sourceAddr);
                break;

            case RERR:
                handleRERR(check_and_cast<BITCARRERR *>(ctrlPacket), sourceAddr);
                break;

            case RREPACK:
                handleRREPACK(check_and_cast<BITCARRREPACK *>(ctrlPacket), sourceAddr);
                break;
...
INetfilter::IHook::Result BITCARRouting::ensureRouteForDatagram(IPv4Datagram *datagram)
{
    ...
    if (destAddr.isLimitedBroadcastAddress() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        BITCARRouteData *routeData = route ? dynamic_cast<BITCARRouteData *>(route->getProtocolData()) : NULL;
...
bool BITCARRouting::hasOngoingRouteDiscovery(const IPv4Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void BITCARRouting::startRouteDiscovery(const IPv4Address& target, unsigned timeToLive)
{
    ...
    BITCARAODVVANETRREQ *rreq = createAODVVANETRREQ(target);
    addressToRreqRetries[target] = 0;
    sendAODVVANETRREQ(rreq, IPv4Address::ALLONES_ADDRESS, timeToLive);
}

IPv4Address BITCARRouting::getSelfIPAddress() const
{
    return routingTable->getRouterId();
}

void BITCARRouting::delayDatagram(IPv4Datagram *datagram)
{
...
void BITCARRouting::sendAODVVANETRREQ(BITCARAODVVANETRREQ *rreq, const IPv4Address& destAddr, unsigned int timeToLive)
{
...
void BITCARRouting::sendRREP(BITCARRREP *rrep, const IPv4Address& destAddr, unsigned int timeToLive)
{
    ...
    BITCARRouteData *destRouteData = check_and_cast<BITCARRouteData *>(destRoute->getProtocolData());
...
BITCARAODVVANETRREQ *BITCARRouting::createAODVVANETRREQ(const IPv4Address& destAddr)
{
    BITCARAODVVANETRREQ *rreqPacket = new BITCARAODVVANETRREQ("BITCAR-AODVVANETRREQ");
    ...
    if (lastKnownRoute && lastKnownRoute->getSource() == this) {
        ...
        BITCARRouteData *routeData = check_and_cast<BITCARRouteData *>(lastKnownRoute->getProtocolData());
...
BITCARRREP *BITCARRouting::createRREP(BITCARAODVVANETRREQ *rreq, IPv4Route *destRoute, IPv4Route *originatorRoute, const IPv4Address& lastHopAddr)
{
...
BITCARRREP *BITCARRouting::createGratuitousRREP(BITCARAODVVANETRREQ *rreq, IPv4Route *originatorRoute)
{
...
void BITCARRouting::handleRREP(BITCARRREP *rrep, const IPv4Address& sourceAddr)
{
    ...
    BITCARRouteData *destRouteData = NULL;
    ...
    if (destRoute && destRoute->getSource() == this) {    // already exists
        destRouteData = check_and_cast<BITCARRouteData *>(destRoute->getProtocolData());
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr()) {
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                BITCARRouteData *nextHopToDestRouteData = check_and_cast<BITCARRouteData *>(nextHopToDestRoute->getProtocolData());
                nextHopToDestRouteData->addPrecursor(originatorRoute->getGateway());

                BITCARRREP *outgoingRREP = rrep->dup();
...
void BITCARRouting::updateRoutingTable(IPv4Route *route, const IPv4Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    ...
    BITCARRouteData *routingData = check_and_cast<BITCARRouteData *>(route->getProtocolData());
...
void BITCARRouting::handleAODVVANETRREQ(BITCARAODVVANETRREQ *rreq, const IPv4Address& sourceAddr, unsigned int timeToLive)
{
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) {    // create
        ...
        BITCARRouteData *routeData = check_and_cast<BITCARRouteData *>(reverseRoute->getProtocolData());
    ...
    BITCARRouteData *destRouteData = destRoute ? dynamic_cast<BITCARRouteData *>(destRoute->getProtocolData()) : NULL;
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        ...
        // create RREP
        BITCARRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
        ...
        destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        if (rreq->getGratuitousRREPFlag()) {
            ...
            BITCARRREP *grrep = createGratuitousRREP(rreq, originatorRoute);
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        BITCARAODVVANETRREQ *outgoingAODVVANETRREQ = rreq->dup();
        forwardAODVVANETRREQ(outgoingAODVVANETRREQ, timeToLive);
    }
    else
        EV_WARN << "Can't forward the AODVVANETRREQ because of its small (<= 1) TTL: " << timeToLive << " or the BITCAR reboot has not completed yet" << endl;

    delete rreq;
}

IPv4Route *BITCARRouting::createRoute(const IPv4Address& destAddr, const IPv4Address& nextHop,
        ...
        bool isActive, simtime_t lifeTime)
{
    ...
    BITCARRouteData *newProtocolData = new BITCARRouteData();
...
void BITCARRouting::receiveChangeNotification(int signalID, const cObject *obj)
{
...
void BITCARRouting::handleLinkBreakSendRERR(const IPv4Address& unreachableAddr)
{
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getGateway() == unreachableAddr) {
            BITCARRouteData *routeData = check_and_cast<BITCARRouteData *>(route->getProtocolData());
...
bool BITCARRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void BITCARRouting::handleWaitForBITCARRREP(WaitForBITCARRREP *rrepTimer)
{
    ...
    BITCARAODVVANETRREQ *rreq = createAODVVANETRREQ(destAddr);
...
void BITCARRouting::forwardRREP(BITCARRREP *rrep, const IPv4Address& destAddr, unsigned int timeToLive)
{
...
void BITCARRouting::forwardAODVVANETRREQ(BITCARAODVVANETRREQ *rreq, unsigned int timeToLive)
{
...
void BITCARRouting::sendGRREP(BITCARRREP *grrep, const IPv4Address& destAddr, unsigned int timeToLive)
{
...
void BITCARRouting::sendHelloMessagesIfNeeded()
{
...
void BITCARRouting::handleHelloMessage(BITCARRREP *helloMessage)
{
    ...
    if (!routeHelloOriginator || routeHelloOriginator->getSource() != this)
        ...
        BITCARRouteData *routeData = check_and_cast<BITCARRouteData *>(routeHelloOriginator->getProtocolData());
...
void BITCARRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result BITCARRouting::datagramForwardHook(IPv4Datagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, IPv4Address& nextHopAddress)
{
    ...
    BITCARRouteData *routeDestData = routeDest ? dynamic_cast<BITCARRouteData *>(routeDest->getProtocolData()) : NULL;
...
void BITCARRouting::sendRERRWhenNoRouteToForward(const IPv4Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    IPv4Route *unreachableRoute = routingTable->findBestMatchingRoute(unreachableAddr);
    BITCARRouteData *unreachableRouteData = unreachableRoute ? dynamic_cast<BITCARRouteData *>(unreachableRoute->getProtocolData()) : NULL;
...
bool BITCARRouting::updateValidRouteLifeTime(const IPv4Address& destAddr, simtime_t lifetime)
{
    IPv4Route *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        BITCARRouteData *routeData = check_and_cast<BITCARRouteData *>(route->getProtocolData());
...
void BITCARRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void BITCARRouting::handleBlackListTimer()
{
