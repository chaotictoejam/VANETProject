...
void NS_CLASS log_init()
{
    ...
    char AODVVANET_LOG_PATH[strlen(AODVVANET_LOG_PATH_PREFIX) +
                       strlen(AODVVANET_LOG_PATH_SUFFIX) + 16];
    char AODVVANET_RT_LOG_PATH[strlen(AODVVANET_LOG_PATH_PREFIX) +
                          strlen(AODVVANET_RT_LOG_PATH_SUFFIX) + 16];


    sprintf(AODVVANET_LOG_PATH, "%s%d%s", AODVVANET_LOG_PATH_PREFIX, node_id,
            AODVVANET_LOG_PATH_SUFFIX);
    sprintf(AODVVANET_RT_LOG_PATH, "%s%d%s", AODVVANET_LOG_PATH_PREFIX, node_id,
            AODVVANET_RT_LOG_PATH_SUFFIX);
    ...
    {
        if ((log_file_fd =
                    open(AODVVANET_LOG_PATH, O_RDWR | O_CREAT | O_TRUNC,
    ...
    {
        if ((log_rt_fd =
                    open(AODVVANET_RT_LOG_PATH, O_RDWR | O_CREAT | O_TRUNC,
                         S_IROTH | S_IWUSR | S_IRUSR | S_IRGRP)) < 0)
        {
