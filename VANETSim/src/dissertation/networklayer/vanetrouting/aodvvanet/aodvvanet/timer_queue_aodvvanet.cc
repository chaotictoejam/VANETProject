...
static void printTQ(avlist_t * l);
#endif
#endif              /* NS_PORT */
...
void NS_CLASS timer_timeout(struct timeval *now)
{
    ...
    avlist_foreach_safe(pos, tmp, &TQ)
    {
        ...
        avlist_detach(&t->l);
        avlist_add_tail(expTQ_ptr, &t->l);
    }

    /* Execute expired timers in expTQ safely by removing them at the head */
    while (!avlist_empty(expTQ_ptr))
    {
        struct timer *t = (struct timer *) avlist_first(expTQ_ptr);
        avlist_detach(&t->l);
        t->used = 0;
#ifdef DEBUG_TIMER_QUEUE
...
#ifdef DEBUG_TIMER_QUEUE
void NS_CLASS printTQ(avlist_t * l)
{
    struct timeval now;
    int n = 0;
    avlist_t *pos;
