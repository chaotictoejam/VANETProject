...
//#endif
    ...
    std::vector<VanetAddress> list;
    getListRelatedAp(dest_addr.s_addr, list);
    for (unsigned int i = 0; i < list.size(); i++)
    {
        struct in_addr auxAaddr;
        auxAaddr.s_addr = list[i];
        if ((rt->state == VALID || rt->state == IMMORTAL)  && avseek_list_remove(avseek_list_find(auxAaddr)))
        {
...
void NS_CLASS rt_table_delete(rt_table_t * rt)
{
    ...
    VanetAddress dest = rt->dest_addr.s_addr;
    AodvRtTableMap::iterator it = aodvRtTableMap.find(dest);
    if (it != aodvRtTableMap.end())
    {
        if (it->second != rt)
                                      ...
                                      u_int16_t flags, unsigned int ifindex)
{
    ...
    VanetAddress dest = dest_addr.s_addr;
...
void precursor_avlist_destroy(rt_table_t * rt)
{
    /* Sanity check */
    if (!rt)
        return;
                                     ...
                                     uint32_t cost,uint8_t hopfix)
{
    ...
    /* Check if we already have an entry for dest_addr */
    avlist_foreach(pos, &rt_tbl.tbl[index])
    {
        rt = (rt_table_t *) pos;
    ...
    rt->nprec = 0;
    INIT_AVLIST_HEAD(&rt->precursors);
    ...
    avlist_add(&rt_tbl.tbl[index], &rt->l);
...
void NS_CLASS precursor_add(rt_table_t * rt, struct in_addr addr)
{
    ...
    avlist_foreach(pos, &rt->precursors)
    {
        pr = (precursor_t *) pos;
...
void precursor_avlist_destroy(rt_table_t * rt)
{
    ...
    avlist_foreach_safe(pos, tmp, &rt->precursors)
    {
        precursor_t *pr = (precursor_t *) pos;
        avlist_detach(pos);
        rt->nprec--;
