...
AODVVANETRERR *NS_CLASS rerr_create(u_int8_t flags,struct in_addr dest_addr,
                           u_int32_t dest_seqno)

{
    AODVVANETRERR *rerr;
    DEBUG(LOG_DEBUG, 0, "Assembling AODVVANETRERR about %s seqno=%d",
          ip_to_str(dest_addr), dest_seqno);
#ifndef OMNETPP
    rerr = (AODVVANETRERR *) aodvvanet_socket_new_msg();
    rerr->dest_addr = dest_addr.s_addr;
    rerr->dest_seqno = htonl(dest_seqno);
#else
    rerr = new AODVVANETRERR("RouteError");
    rerr->addUdest (dest_addr.s_addr,htonl(dest_seqno));
    totalRerrSend++;
#endif
    rerr->type = AODVVANET_AODVVANETRERR;
    rerr->n = (flags & AODVVANETRERR_NODELETE ? 1 : 0);
    rerr->res1 = 0;
    rerr->res2 = 0;
    rerr->dest_count = 1;
    return rerr;
...
void NS_CLASS rerr_add_udest(AODVVANETRERR * rerr,struct in_addr udest,
                             u_int32_t udest_seqno)
{
#ifndef OMNETPP
    AODVVANETRERR_udest *ud;
    ud = (AODVVANETRERR_udest *) ((char *) rerr + AODVVANETRERR_CALC_SIZE(rerr));
...
void NS_CLASS rerr_process(AODVVANETRERR * rerr, int rerrlen,struct in_addr ip_src,
                           struct in_addr ip_dst)
{
    AODVVANETRERR *new_rerr = NULL;
    AODVVANETRERR_udest *udest;
    rt_table_t *rt;
    ...
    udest = AODVVANETRERR_UDEST_FIRST(rerr);
    ...
    while (rerr->dest_count)
    {
        ...
        if (rt && rt->state == VALID && rt->next_hop.s_addr == ip_src.s_addr)
        {
             ...
             *       * addition to AODV-VANET. It is here because it makes a lot
             *       * of sense... */
            if (0 && (int32_t) rt->dest_seqno > (int32_t) rerr_dest_seqno)
            {
                DEBUG(LOG_DEBUG, 0, "Udest ignored because of seqno");
                udest = AODVVANETRERR_UDEST_NEXT(udest);
                rerr->dest_count--;
                continue;
            }
            DEBUG(LOG_DEBUG, 0, "removing rte %s - WAS IN AODVVANETRERR!!",
            ...
            if (rt->nprec && !(rt->flags & RT_REPAIR))
            {
                ...
                {
                    /* Decide whether new precursors make this a non unicast AODVVANETRERR */
                    ...
                    {
                        avlist_t *pos2;
                        avlist_foreach(pos2, &rt->precursors)
                        {
            ...
            {
                DEBUG(LOG_DEBUG, 0,
                      "Not sending AODVVANETRERR, no precursors or route in RT_REPAIR");
            }
    ...
    /* If a AODVVANETRERR was created, then send it now... */
    if (new_rerr)
    {
        ...
        if (rt && new_rerr->dest_count == 1 && !rerr_unicast_dest.s_addr.isUnspecified())
            aodvvanet_socket_send((AODVVANET_msg *) new_rerr,
                             rerr_unicast_dest,
                             AODVVANETRERR_CALC_SIZE(new_rerr), 1,
                             &DEV_IFINDEX(rt->ifindex));

        else if (new_rerr->dest_count > 0)
        {
            /* FIXME: Should only transmit AODVVANETRERR on those interfaces
            ...
            for (i = 0; i < MAX_NR_INTERFACES; i++)
            {
                ...
                if (numInterfaces>1)
                {
                    aodvvanet_socket_send((AODVVANET_msg *) new_rerr->dup(), dest,AODVVANETRERR_CALC_SIZE(new_rerr), 1, &DEV_NR(i));
                }
                else
#endif
                    aodvvanet_socket_send((AODVVANET_msg *) new_rerr, dest,AODVVANETRERR_CALC_SIZE(new_rerr), 1, &DEV_NR(i));
                numInterfaces--;
            }
