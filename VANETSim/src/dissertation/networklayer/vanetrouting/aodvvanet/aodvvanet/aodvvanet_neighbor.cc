...
void NS_CLASS neighbor_add(AODVVANET_msg * aodvvanet_msg, struct in_addr source,
...
void NS_CLASS neighbor_link_break(rt_table_t * rt)
{
    /* If hopcount = 1, this is a direct neighbor and a link break has
       occured. Send a AODVVANETRERR with the incremented sequence number */
    AODVVANETRERR *rerr = NULL;
    rt_table_t *rt_u;
    ...
    /* Check the routing table for entries which have the unreachable
       ...
       therefore also be included in the AODVVANETRERR. */
    for (AodvRtTableMap::iterator it = aodvRtTableMap.begin(); it != aodvRtTableMap.end(); it++)
    {
                ...
                rt_u->dest_addr.s_addr != rt->dest_addr.s_addr)
        {
            ...
            precursor_avlist_destroy(rt_u);
        }
    }

    if (rerr)
    {
        DEBUG(LOG_DEBUG, 0, "AODVVANETRERR created, %d bytes.", AODVVANETRERR_CALC_SIZE(rerr));
            ...
            for (i = 0; i < MAX_NR_INTERFACES; i++)
            {
                ...
                if (cont>1)
                    aodvvanet_socket_send((AODVVANET_msg *) rerr->dup(), dest,
                                     AODVVANETRERR_CALC_SIZE(rerr), 1, &DEV_NR(i),delay);
                else
...
void NS_CLASS neighbor_link_break(rt_table_t * rt)
{
    ...
    {
        avlist_t *pos;
        avlist_foreach(pos, &rt_tbl.tbl[i])
        {
                    ...
                    rt_u->dest_addr.s_addr != rt->dest_addr.s_addr)
            {
                ...
                if (rt_u->nprec)
                {
                    ...
                    else
                    {
                        ...
                        {
                            avlist_t *pos2;
                            avlist_foreach(pos2, &rt_u->precursors)
                            {
    ...
    if (rerr)
    {
        ...
        else if (rerr->dest_count > 0)
        {
            ...
            for (i = 0; i < MAX_NR_INTERFACES; i++)
            {
                ...
                if (!DEV_NR(i).enabled)
                    continue;
                dest.s_addr = AODVVANET_BROADCAST;
#ifdef OMNETPP
                if (cont>1)
                    aodvvanet_socket_send((AODVVANET_msg *) rerr->dup(), dest,
                                     AODVVANETRERR_CALC_SIZE(rerr), 1, &DEV_NR(i),delay);
...
#else
                aodvvanet_socket_send((AODVVANET_msg *) rerr, dest,
                                 AODVVANETRERR_CALC_SIZE(rerr), 1, &DEV_NR(i));
#endif
            }
        }
#ifdef OMNETPP
