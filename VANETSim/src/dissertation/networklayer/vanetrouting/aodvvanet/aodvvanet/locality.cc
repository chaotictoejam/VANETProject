...
int NS_CLASS locality(struct in_addr dest, unsigned int ifindex)
{
    ...
    dstnet.s_addr = VanetAddress(dest.s_addr); dstnet.s_addr.setPrefix(masklen);   // dstnet.s_addr = dest.s_addr & mask.s_addr;
    interface.s_addr = VanetAddress(ie->ipv4Data()->getIPAddress());
