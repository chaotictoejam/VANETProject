...
void NS_CLASS packet_queue_destroy(void)
{
    ...
    avlist_foreach_safe(pos, tmp, &PQ.head)
    {
...
void NS_CLASS packet_queue_add(cPacket * p, struct in_addr dest_addr)
{
    ...
    if (PQ.len >= MAX_QUEUE_LENGTH)
    {
        ...
        if (!avlist_empty(&PQ.head))
        {
            qp = (struct q_pkt *)PQ.head.next;
            avlist_detach(PQ.head.next);
            dgram = qp->p;
            // drop (dgram);
            sendICMP(dgram);
//  icmpAccess.get()->sendErrorMessage(dgram, ICMP_DESTINATION_UNREACHABLE, ICMP_AODVVANET_QUEUE_FULL);
            free(qp);
            PQ.len--;
        }
    }
...
#define ARP_DELAY 0.005
    ...
    if (verdict == PQ_ENC_SEND)
    {
        gw_addr.s_addr = VanetAddress(*gateWayAddress);
        rt = rt_table_find(gw_addr);
    }
    else
        rt = rt_table_find(dest_addr);
    ...
    if (isInMacLayer())
    {
        ...
        while (!listMac.empty())
        {
            list.push_back(VanetAddress(listMac.back()));
            listMac.pop_back();
        }
    }
    else
    {
        ...
        while (!listIp.empty())
        {
            list.push_back(VanetAddress(listIp.back()));
            listIp.pop_back();
        }
    }
