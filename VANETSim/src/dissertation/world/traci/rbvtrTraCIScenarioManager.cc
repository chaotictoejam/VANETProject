...
Define_Module(rbvtrTraCIScenarioManager);

rbvtrTraCIScenarioManager::~rbvtrTraCIScenarioManager()
{
    cancelAndDelete(connectAndStartTrigger);
    cancelAndDelete(executeOneTimestepTrigger);
}

void rbvtrTraCIScenarioManager::initialize(int stage)
{
    ...
    while (std::getline(roiRects_i, rect, ' '))
    {
        ...
        rect_i >> y2;
        ASSERT(rect_i);
        roiRects.push_back(std::pair<rbvtrTraCICoord, rbvtrTraCICoord>(rbvtrTraCICoord(x1, y1), rbvtrTraCICoord(x2, y2)));
    }
    ...
    MYDEBUG << "initialized rbvtrTraCIScenarioManager" << endl;
}

std::string rbvtrTraCIScenarioManager::receiverbvtrTraCIMessage()
{
    if (!socketPtr)
        error("Connection to rbvtrTraCI server lost");
    ...
    uint32_t msgLength;
    {
        ...
        while (bytesRead < sizeof(uint32_t))
        {
            ...
            {
                error("Connection to rbvtrTraCI server closed unexpectedly. Check your server's log");
            }
            else
            {
                if (sock_errno() == EINTR)
                ...
                if (sock_errno() == EAGAIN)
                    continue;
                error("Connection to rbvtrTraCI server lost. Check your server's log. Error message: %d: %s", sock_errno(),
                        strerror(sock_errno()));
            }
        }
        rbvtrTraCIBuffer(std::string(buf2, sizeof(uint32_t))) >> msgLength;
    }

    uint32_t bufLength = msgLength - sizeof(msgLength);
    char buf[bufLength];
    {
        MYDEBUG << "Reading rbvtrTraCI message of " << bufLength << " bytes" << endl;
        uint32_t bytesRead = 0;
...
void rbvtrTraCIScenarioManager::sendrbvtrTraCIMessage(std::string buf)
{
    if (!socketPtr)
        error("Connection to rbvtrTraCI server lost");

    {
        ...
        rbvtrTraCIBuffer buf2 = rbvtrTraCIBuffer();
        buf2 << msgLength;
    ...
    {
        MYDEBUG << "Writing rbvtrTraCI message of " << buf.length() << " bytes" << endl;
        uint32_t bytesWritten = 0;
...
std::string rbvtrTraCIScenarioManager::makerbvtrTraCICommand(uint8_t commandId, rbvtrTraCIBuffer buf)
{
    if (sizeof(uint8_t) + sizeof(uint8_t) + buf.str().length() > 0xFF)
    {
        ...
        return (rbvtrTraCIBuffer() << static_cast<uint8_t>(0) << len << commandId).str() + buf.str();
    }
    uint8_t len = sizeof(uint8_t) + sizeof(uint8_t) + buf.str().length();
    return (rbvtrTraCIBuffer() << len << commandId).str() + buf.str();
}

rbvtrTraCIScenarioManager::rbvtrTraCIBuffer rbvtrTraCIScenarioManager::queryrbvtrTraCI(uint8_t commandId, const rbvtrTraCIBuffer& buf)
{
    sendrbvtrTraCIMessage(makerbvtrTraCICommand(commandId, buf));
    ...
    if (result == RTYPE_NOTIMPLEMENTED)
        error("rbvtrTraCI server reported command 0x%2x not implemented (\"%s\"). Might need newer version.", commandId,
                description.c_str());
    if (result == RTYPE_ERR)
        error("rbvtrTraCI server reported error executing command 0x%2x (\"%s\").", commandId, description.c_str());
    ASSERT(result == RTYPE_OK);
    return obuf;
}

rbvtrTraCIScenarioManager::rbvtrTraCIBuffer rbvtrTraCIScenarioManager::queryrbvtrTraCIOptional(uint8_t commandId, const rbvtrTraCIBuffer& buf,
...
void rbvtrTraCIScenarioManager::connect()
{
    MYDEBUG << "rbvtrTraCIScenarioManager connecting to rbvtrTraCI server" << endl;
    ...
    {
        error("Invalid rbvtrTraCI server address: %s", host.c_str());
        return;
    }
    ...
    if (MYSOCKET < 0)
        error("Could not create socket to connect to rbvtrTraCI server");

    if (::connect(MYSOCKET, (sockaddr const*) &address, sizeof(address)) < 0)
    {
        error(
                "Could not connect to rbvtrTraCI server. Make sure it is running and not behind a firewall. Error message: %d: %s",
...
void rbvtrTraCIScenarioManager::init_traci()
{
    {
        ...
        {
            MYDEBUG << "rbvtrTraCI server \"" << serverVersion << "\" reports API version " << apiVersion << endl;
        }
        else
        {
            error("rbvtrTraCI server \"%s\" reports API version %d. This server is unsupported.", serverVersion.c_str(),
                    apiVersion);
        }
...

    {
        ...
        rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_GET_SIM_VARIABLE,
                rbvtrTraCIBuffer() << static_cast<uint8_t>(VAR_NET_BOUNDING_BOX) << std::string("sim0"));
        ...
        netbounds1 = rbvtrTraCICoord(x1, y1);
        netbounds2 = rbvtrTraCICoord(x2, y2);
        MYDEBUG << "rbvtrTraCI reports network boundaries (" << x1 << ", " << y1 << ")-(" << x2 << ", " << y2 << ")" << endl;
    }

    {
        ...
        rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SUBSCRIBE_SIM_VARIABLE,
                rbvtrTraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2
...

    {
        ...
        rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SUBSCRIBE_VEHICLE_VARIABLE,
                rbvtrTraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1);
        processSubcriptionResult(buf);
        ASSERT(buf.eof());
    }
...
void rbvtrTraCIScenarioManager::finish()
{
...
void rbvtrTraCIScenarioManager::handleMessage(cMessage *msg)
{
    ...
    error("rbvtrTraCIScenarioManager doesn't handle messages from other modules");
}

void rbvtrTraCIScenarioManager::handleSelfMsg(cMessage *msg)
{
    ...
    error("rbvtrTraCIScenarioManager received unknown self-message");
}

std::pair<uint32_t, std::string> rbvtrTraCIScenarioManager::commandGetVersion()
{
    bool success = false;
    rbvtrTraCIBuffer buf = queryrbvtrTraCIOptional(CMD_GETVERSION, rbvtrTraCIBuffer(), success);
...
void rbvtrTraCIScenarioManager::commandSetSpeedMode(std::string nodeId, int32_t bitset)
{
    uint8_t variableId = VAR_SPEEDSETMODE;
    uint8_t variableType = TYPE_INTEGER;
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << variableId << nodeId << variableType << bitset);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::commandSetSpeed(std::string nodeId, double speed)
{
    uint8_t variableId = VAR_SPEED;
    uint8_t variableType = TYPE_DOUBLE;
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << variableId << nodeId << variableType << speed);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::commandNewRoute(std::string nodeId, std::string roadId)
{
    uint8_t variableId = LANE_EDGE_ID;
    uint8_t variableType = TYPE_STRING;
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << variableId << nodeId << variableType << roadId);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::commandSetVehicleParking(std::string nodeId)
{
    uint8_t variableId = REMOVE;
    uint8_t variableType = TYPE_BYTE;
    uint8_t value = REMOVE_PARKING;
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << variableId << nodeId << variableType << value);
    ASSERT(buf.eof());
}

std::string rbvtrTraCIScenarioManager::commandGetEdgeId(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_ROAD_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string rbvtrTraCIScenarioManager::commandGetCurrentEdgeOnRoute(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, LANE_EDGE_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string rbvtrTraCIScenarioManager::commandGetLaneId(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_LANE_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

double rbvtrTraCIScenarioManager::commandGetLanePosition(std::string nodeId)
{
    return genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_LANEPOSITION,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::list<std::string> rbvtrTraCIScenarioManager::commandGetPlannedEdgeIds(std::string nodeId)
{
    return genericGetStringList(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_EDGES,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string rbvtrTraCIScenarioManager::commandGetRouteId(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_ROUTE_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::list<std::string> rbvtrTraCIScenarioManager::commandGetRouteEdgeIds(std::string routeId)
{
    return genericGetStringList(CMD_GET_ROUTE_VARIABLE, routeId, VAR_EDGES,
    RESPONSE_GET_ROUTE_VARIABLE);
}

void rbvtrTraCIScenarioManager::commandChangeRoute(std::string nodeId, std::string roadId, double travelTime)
{
    if (travelTime >= 0)
    {
        ...
        rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
                rbvtrTraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId << newTimeT
                        << newTime);
        ASSERT(buf.eof());
    }
    else
    {
        ...
        std::string edgeId = roadId;
        rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
                rbvtrTraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId);
        ASSERT(buf.eof());
    }
    {
        ...
        int32_t count = 0;
        rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
                rbvtrTraCIBuffer() << variableId << nodeId << variableType << count);
        ASSERT(buf.eof());
    }
}

double rbvtrTraCIScenarioManager::commandDistanceRequest(Coord position1, Coord position2, bool returnDrivingDistance)
{
    uint8_t variable = DISTANCE_REQUEST;
    std::string simId = "sim0";
    uint8_t variableType = TYPE_COMPOUND;
    int32_t count = 3;
    rbvtrTraCICoord p1 = omnet2rbvtrTraCI(position1);
    rbvtrTraCICoord p2 = omnet2rbvtrTraCI(position2);
    uint8_t dType = static_cast<uint8_t>(returnDrivingDistance ? REQUEST_DRIVINGDIST : REQUEST_AIRDIST);

    // query road network boundaries
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_GET_SIM_VARIABLE,
            rbvtrTraCIBuffer() << variable << simId << variableType << count << p1 << p2 << dType);
    uint8_t cmdLength_resp;
    buf >> cmdLength_resp;
    uint8_t commandId_resp;
    buf >> commandId_resp;
...
void rbvtrTraCIScenarioManager::commandStopNode(std::string nodeId, std::string roadId, double pos, uint8_t laneid,
        double radius, double waittime)
{
    ...
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId << stopPosT << stopPos
                    << stopLaneT << stopLane << durationT << duration);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::commandSetTrafficLightProgram(std::string trafficLightId, std::string program)
{
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_TL_VARIABLE,
            rbvtrTraCIBuffer() << static_cast<uint8_t>(TL_PROGRAM) << trafficLightId << static_cast<uint8_t>(TYPE_STRING)
                    << program);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::commandSetTrafficLightPhaseIndex(std::string trafficLightId, int32_t index)
{
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_TL_VARIABLE,
            rbvtrTraCIBuffer() << static_cast<uint8_t>(TL_PHASE_INDEX) << trafficLightId
                    << static_cast<uint8_t>(TYPE_INTEGER) << index);
    ASSERT(buf.eof());
}

std::list<std::string> rbvtrTraCIScenarioManager::commandGetPolygonIds()
{
    return genericGetStringList(CMD_GET_POLYGON_VARIABLE, "", ID_LIST, RESPONSE_GET_POLYGON_VARIABLE);
}

std::string rbvtrTraCIScenarioManager::commandGetPolygonTypeId(std::string polyId)
{
    return genericGetString(CMD_GET_POLYGON_VARIABLE, polyId, VAR_TYPE, RESPONSE_GET_POLYGON_VARIABLE);
}

std::list<Coord> rbvtrTraCIScenarioManager::commandGetPolygonShape(std::string polyId)
{
    return genericGetCoordList(CMD_GET_POLYGON_VARIABLE, polyId, VAR_SHAPE, RESPONSE_GET_POLYGON_VARIABLE);
}

void rbvtrTraCIScenarioManager::commandSetPolygonShape(std::string polyId, std::list<Coord> points)
{
    ...
    {
        rbvtrTraCICoord pos = omnet2rbvtrTraCI(*i);
        buf << static_cast<double>(pos.x) << static_cast<double>(pos.y);
    }
    rbvtrTraCIBuffer obuf = queryrbvtrTraCI(CMD_SET_POLYGON_VARIABLE, buf);
    ASSERT(obuf.eof());
}

void rbvtrTraCIScenarioManager::commandAddPolygon(std::string polyId, std::string polyType, const vanetTraCIColor& color,
        bool filled, int32_t layer, std::list<Coord> points)
{
    ...
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_POLYGON_VARIABLE, p);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::commandRemovePolygon(std::string polyId, int32_t layer)
{
    rbvtrTraCIBuffer p;
...
void rbvtrTraCIScenarioManager::commandAddPoi(std::string poiId, std::string poiType, const vanetTraCIColor& color, int32_t layer,
        Coord pos)
{
    rbvtrTraCIBuffer p;

    rbvtrTraCICoord rbvtrTraCIPos = omnet2rbvtrTraCI(pos);
    ...
    p << static_cast<uint8_t>(TYPE_COLOR) << color.red << color.green << color.blue << color.alpha;
    p << static_cast<uint8_t>(TYPE_INTEGER) << layer;
    p << static_cast<uint8_t>(POSITION_2D) << rbvtrTraCIPos.x << rbvtrTraCIPos.y;

    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SET_POI_VARIABLE, p);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::commandRemovePoi(std::string poiId, int32_t layer)
{
    rbvtrTraCIBuffer p;
        ...
        simtime_t emitTime_st, double emitPosition, double emitSpeed, int8_t emitLane)
{
    ...
    rbvtrTraCIBuffer buf = queryrbvtrTraCIOptional(CMD_SET_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << variableId << vehicleId << variableType << count << (uint8_t) TYPE_STRING << vehicleTypeId
...
void rbvtrTraCIScenarioManager::addModule(std::string nodeId, std::string type, std::string name, std::string displayString,
        const Coord& position, std::string road_id, double speed, double acceleration, double angle)
{
    ...
    // pre-initialize rbvtrTraCIMobility
    for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++)
    {
        cModule* submod = iter();
        rbvtrTraCIMobility* mm = dynamic_cast<rbvtrTraCIMobility*>(submod);
        if (!mm)
            continue;
    ...
    // post-initialize rbvtrTraCIMobility
    for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++)
    {
        cModule* submod = iter();
        rbvtrTraCIMobility* mm = dynamic_cast<rbvtrTraCIMobility*>(submod);
...
cModule* rbvtrTraCIScenarioManager::getManagedModule(std::string nodeId)
{
    if (hosts.find(nodeId) == hosts.end())
        return 0;
    return hosts[nodeId];
...
bool rbvtrTraCIScenarioManager::isModuleUnequipped(std::string nodeId)
{
...
void rbvtrTraCIScenarioManager::deleteModule(std::string nodeId)
{
...
bool rbvtrTraCIScenarioManager::isInRegionOfInterest(const rbvtrTraCICoord& position, std::string road_id, double speed,
        double angle)
{
    ...
    if (roiRects.size() > 0)
    {
        for (std::list<std::pair<rbvtrTraCICoord, rbvtrTraCICoord> >::const_iterator i = roiRects.begin(); i != roiRects.end();
                ++i)
        {
...
uint32_t rbvtrTraCIScenarioManager::getCurrentTimeMs()
{
    return static_cast<uint32_t>(round(simTime().dbl() * 1000));
}

void rbvtrTraCIScenarioManager::executeOneTimestep()
{

    MYDEBUG << "Triggering rbvtrTraCI server simulation advance to t=" << simTime() << endl;

    uint32_t targetTime = getCurrentTimeMs();

    if (targetTime > round(connectAt.dbl() * 1000))
    {
        rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SIMSTEP2, rbvtrTraCIBuffer() << targetTime);
...
std::string rbvtrTraCIScenarioManager::genericGetString(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
    ...
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(commandId, rbvtrTraCIBuffer() << variableId << objectId);
...
Coord rbvtrTraCIScenarioManager::genericGetCoord(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
    ...
    return rbvtrTraCI2omnet(rbvtrTraCICoord(x, y));
}

double rbvtrTraCIScenarioManager::genericGetDouble(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
...
std::list<std::string> rbvtrTraCIScenarioManager::genericGetStringList(uint8_t commandId, std::string objectId,
...
std::list<Coord> rbvtrTraCIScenarioManager::genericGetCoordList(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        double y;
        buf >> y;
        Coord pos = rbvtrTraCI2omnet(rbvtrTraCICoord(x, y));
        res.push_back(pos);
    }
...
Coord rbvtrTraCIScenarioManager::rbvtrTraCI2omnet(rbvtrTraCICoord coord) const
{
    return Coord(coord.x - netbounds1.x + margin, (netbounds2.y - netbounds1.y) - (coord.y - netbounds1.y) + margin);
}

rbvtrTraCIScenarioManager::rbvtrTraCICoord rbvtrTraCIScenarioManager::omnet2rbvtrTraCI(Coord coord) const
{
    return rbvtrTraCICoord(coord.x + netbounds1.x - margin,
            (netbounds2.y - netbounds1.y) - (coord.y - netbounds1.y) + margin);
}

double rbvtrTraCIScenarioManager::rbvtrTraCI2omnetAngle(double angle) const
{

    // rotate angle so 0 is east (in rbvtrTraCI's angle interpretation 0 is south)
    angle = angle - 90;
...
double rbvtrTraCIScenarioManager::omnet2rbvtrTraCIAngle(double angle) const
{
...
void rbvtrTraCIScenarioManager::subscribeToVehicleVariables(std::string vehicleId)
{
    ...
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SUBSCRIBE_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2 << variable3
                    << variable4 << variable5);
    processSubcriptionResult(buf);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::unsubscribeFromVehicleVariables(std::string vehicleId)
{
    // subscribe to some attributes of the vehicle
    uint32_t beginTime = 0;
    uint32_t endTime = 0x7FFFFFFF;
    ...
    rbvtrTraCIBuffer buf = queryrbvtrTraCI(CMD_SUBSCRIBE_VEHICLE_VARIABLE,
            rbvtrTraCIBuffer() << beginTime << endTime << objectId << variableNumber);
    ASSERT(buf.eof());
}

void rbvtrTraCIScenarioManager::processSimSubscription(std::string objectId, rbvtrTraCIBuffer& buf)
{
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            if (isokay == RTYPE_NOTIMPLEMENTED)
                error(
                        "rbvtrTraCI server reported subscribing to variable 0x%2x not implemented (\"%s\"). Might need newer version.",
                        variable1_resp, description.c_str());
            error("rbvtrTraCI server reported error subscribing to variable 0x%2x (\"%s\").", variable1_resp,
                    description.c_str());
        }
        ...
        if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "rbvtrTraCI reports " << count << " departed vehicles." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_ARRIVED_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "rbvtrTraCI reports " << count << " arrived vehicles." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_TELEPORT_STARTING_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "rbvtrTraCI reports " << count << " vehicles starting to teleport." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_TELEPORT_ENDING_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "rbvtrTraCI reports " << count << " vehicles ending teleport." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_TIME_STEP)
        {
            ...
            buf >> serverTimestep;
            MYDEBUG << "rbvtrTraCI reports current time step as " << serverTimestep << "ms." << endl;
...
void rbvtrTraCIScenarioManager::processVehicleSubscription(std::string objectId, rbvtrTraCIBuffer& buf)
{
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            if (isSubscribed)
            {
                if (isokay == RTYPE_NOTIMPLEMENTED)
                    error(
                            "rbvtrTraCI server reported subscribing to vehicle variable 0x%2x not implemented (\"%s\"). Might need newer version.",
                            variable1_resp, errormsg.c_str());
                error("rbvtrTraCI server reported error subscribing to vehicle variable 0x%2x (\"%s\").", variable1_resp,
                        errormsg.c_str());
            }
        }
        else if (variable1_resp == ID_LIST)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "rbvtrTraCI reports " << count << " active vehicles." << endl;
    ...
    Coord p = rbvtrTraCI2omnet(rbvtrTraCICoord(px, py));
    if ((p.x < 0) || (p.y < 0))
        error("received bad node position (%.2f, %.2f), translated to (%.2f, %.2f)", px, py, p.x, p.y);

    double angle = rbvtrTraCI2omnetAngle(angle_rbvtrTraCI);

    cModule* mod = getManagedModule(objectId);

    // is it in the ROI?
    bool inRoi = isInRegionOfInterest(rbvtrTraCICoord(px, py), edge, speed, angle);
    if (!inRoi)
    {
        if (mod)
        {
...
void rbvtrTraCIScenarioManager::processSubcriptionResult(rbvtrTraCIBuffer& buf)
{
    uint8_t cmdLength_resp;
    buf >> cmdLength_resp;
...
template<> void rbvtrTraCIScenarioManager::rbvtrTraCIBuffer::write(rbvtrTraCICoord inv)
{
