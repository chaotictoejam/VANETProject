...
rt_table_t *NS_CLASS modifyAODVVANETTables(struct in_addr dest_addr,
                                      struct in_addr next,
                                      u_int8_t hops, u_int32_t seqno,
                                      u_int32_t life, u_int8_t state,
                                      u_int16_t flags, unsigned int ifindex)
{
    ...
    DEBUG(LOG_INFO, 0, "modifyAODVVANETTables");
    /* Check if we already have an entry for dest_addr */
...
#else
        // Added in version aodvvanet 0.9.3, it's commented for performance reasons
        //  neighbor_link_break(rt);
#endif
        ...
        rt_table_t * rev_rt)
{
    ...
    /* When forwarding a packet, we update the lifetime of the
       ...
       next hop neighbor (if not the same). AODVVANET draft 10, section
       6.2. */
