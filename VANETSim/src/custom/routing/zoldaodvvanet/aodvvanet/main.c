...
#include "nl_aodvvanet.h"

#ifdef LLFEEDBACK
#include "llf_aodvvanet.h"
#endif
...
void load_modules(char *ifname)
{
    ...
    if (stat("./kaodvvanet.ko", &st) == 0)
	sprintf(buf, "/sbin/insmod kaodvvanet.ko ifname=%s &>/dev/null", ifname);
    else if (stat("./kaodvvanet.o", &st) == 0)
	sprintf(buf, "/sbin/insmod kaodvvanet.o ifname=%s &>/dev/null", ifname);
    else
	...
	if (!strcmp(l, "ipchains")) {
	    fprintf(stderr,
		    "The ipchains kernel module is loaded and prevents AODVVANET-UU from functioning properly.\n");
	    exit(-1);
	}
    }
    fclose(m);
...
int main(int argc, char **argv)
{
	...
	case 'V':
	    printf
		("\nAODVVANET-UU v%s, %s © Uppsala University & Ericsson AB.\nAuthor: Erik Nordström, <erik.nordstrom@it.uu.se>\n\n",
