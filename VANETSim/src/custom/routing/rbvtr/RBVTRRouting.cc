...
Define_Module(RBVTRRouting);

void RBVTRRouting::initialize(int stage)
{
    if (stage == 0) {
...
void RBVTRRouting::handleMessage(cMessage *msg)
{
    if (!isOperational) {
    ...
    if (msg->isSelfMessage()) {
        ...
        RBVTRControlPacket *ctrlPacket = check_and_cast<RBVTRControlPacket *>(udpPacket->decapsulate());
        ...
        switch (ctrlPacket->getPacketType()) {
            ...
            case RREP:
                handleRREP(check_and_cast<RBVTRRREP *>(ctrlPacket), sourceAddr);
                break;

            case RERR:
                handleRERR(check_and_cast<RBVTRRERR *>(ctrlPacket), sourceAddr);
                break;

            case RREPACK:
                handleRREPACK(check_and_cast<RBVTRRREPACK *>(ctrlPacket), sourceAddr);
                break;
...
INetfilter::IHook::Result RBVTRRouting::ensureRouteForDatagram(IPv4Datagram *datagram)
{
    ...
    if (destAddr.isLimitedBroadcastAddress() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        RBVTRRouteData *routeData = route ? dynamic_cast<RBVTRRouteData *>(route->getProtocolData()) : NULL;
...
bool RBVTRRouting::hasOngoingRouteDiscovery(const IPv4Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void RBVTRRouting::startRouteDiscovery(const IPv4Address& target, unsigned timeToLive)
{
...
void RBVTRRouting::delayDatagram(IPv4Datagram *datagram)
{
...
void RBVTRRouting::sendRREP(RBVTRRREP *rrep, const IPv4Address& destAddr, unsigned int timeToLive)
{
    ...
    RBVTRRouteData *destRouteData = check_and_cast<RBVTRRouteData *>(destRoute->getProtocolData());
...
RBVTRRREQ *RBVTRRouting::createRREQ(const IPv4Address& destAddr)
{
    ...
    if (lastKnownRoute && lastKnownRoute->getSource() == this) {
        ...
        RBVTRRouteData *routeData = check_and_cast<RBVTRRouteData *>(lastKnownRoute->getProtocolData());
...
RBVTRRREP *RBVTRRouting::createRREP(RBVTRRREQ *rreq, IPv4Route *destRoute, IPv4Route *originatorRoute, const IPv4Address& lastHopAddr)
{
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {    // node is itself the requested destination
        ...
        RBVTRRouteData *destRouteData = check_and_cast<RBVTRRouteData *>(destRoute->getProtocolData());
        RBVTRRouteData *originatorRouteData = check_and_cast<RBVTRRouteData *>(originatorRoute->getProtocolData());
...
void RBVTRRouting::handleRREP(RBVTRRREP *rrep, const IPv4Address& sourceAddr)
{
    ...
    RBVTRRouteData *destRouteData = NULL;
    ...
    if (destRoute && destRoute->getSource() == this) {    // already exists
        destRouteData = check_and_cast<RBVTRRouteData *>(destRoute->getProtocolData());
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                if (rrep->getAckRequiredFlag()) {
                    RBVTRRREPACK *rrepACK = createRREPACK();
                ...
                RBVTRRouteData *nextHopToDestRouteData = check_and_cast<RBVTRRouteData *>(nextHopToDestRoute->getProtocolData());
                nextHopToDestRouteData->addPrecursor(originatorRoute->getGateway());

                RBVTRRREP *outgoingRREP = rrep->dup();
...
void RBVTRRouting::updateRoutingTable(IPv4Route *route, const IPv4Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    ...
    RBVTRRouteData *routingData = check_and_cast<RBVTRRouteData *>(route->getProtocolData());
...
void RBVTRRouting::handleRREQ(RBVTRRREQ *rreq, const IPv4Address& sourceAddr, unsigned int timeToLive)
{
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) {    // create
        ...
        RBVTRRouteData *routeData = check_and_cast<RBVTRRouteData *>(reverseRoute->getProtocolData());
    ...
    RBVTRRouteData *destRouteData = destRoute ? dynamic_cast<RBVTRRouteData *>(destRoute->getProtocolData()) : NULL;
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        ...
        // create RREP
        RBVTRRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
...
IPv4Route *RBVTRRouting::createRoute(const IPv4Address& destAddr, const IPv4Address& nextHop,
        ...
        bool isActive, simtime_t lifeTime)
{
    ...
    RBVTRRouteData *newProtocolData = new RBVTRRouteData();
...
void RBVTRRouting::receiveChangeNotification(int signalID, const cObject *obj)
{
...
void RBVTRRouting::handleLinkBreakSendRERR(const IPv4Address& unreachableAddr)
{
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getGateway() == unreachableAddr) {
            RBVTRRouteData *routeData = check_and_cast<RBVTRRouteData *>(route->getProtocolData());
...
bool RBVTRRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void RBVTRRouting::sendGRREP(RBVTRRREP *grrep, const IPv4Address& destAddr, unsigned int timeToLive)
{
...
void RBVTRRouting::sendHelloMessagesIfNeeded()
{
...
void RBVTRRouting::handleHelloMessage(RBVTRRREP *helloMessage)
{
    ...
    if (!routeHelloOriginator || routeHelloOriginator->getSource() != this)
        ...
        RBVTRRouteData *routeData = check_and_cast<RBVTRRouteData *>(routeHelloOriginator->getProtocolData());
...
void RBVTRRouting::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IPv4Route *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void RBVTRRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result RBVTRRouting::datagramForwardHook(IPv4Datagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, IPv4Address& nextHopAddress)
{
    ...
    RBVTRRouteData *routeDestData = routeDest ? dynamic_cast<RBVTRRouteData *>(routeDest->getProtocolData()) : NULL;
...
void RBVTRRouting::sendRERRWhenNoRouteToForward(const IPv4Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    IPv4Route *unreachableRoute = routingTable->findBestMatchingRoute(unreachableAddr);
    RBVTRRouteData *unreachableRouteData = unreachableRoute ? dynamic_cast<RBVTRRouteData *>(unreachableRoute->getProtocolData()) : NULL;
...
bool RBVTRRouting::updateValidRouteLifeTime(const IPv4Address& destAddr, simtime_t lifetime)
{
    IPv4Route *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        RBVTRRouteData *routeData = check_and_cast<RBVTRRouteData *>(route->getProtocolData());
...
void RBVTRRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void RBVTRRouting::handleBlackListTimer()
{
