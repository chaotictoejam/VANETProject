...
#ifndef WORLD_TraCI_aodvTraCISCENARIOMANAGER_H
#define WORLD_TraCI_aodvTraCISCENARIOMANAGER_H
...
/**
 * @brief
 * Creates and moves nodes controlled by a aodvTraCI server.
 *
 * If the server is a SUMO road traffic simulation, you can use the
 * aodvTraCIScenarioManagerLaunchd module and sumo-launchd.py script instead.
 *
 * All nodes created thus must have a aodvTraCIMobility submodule.
 *
...
class aodvTraCIScenarioManager : public cSimpleModule
{
    ...
    protected:
        /**
         * Coord equivalent for storing aodvTraCI coordinates
         */
        struct aodvTraCICoord {
            aodvTraCICoord() : x(0), y(0) {}
            aodvTraCICoord(double x, double y) : x(x), y(y) {}
            double x;
            double y;
        };

        /**
         * Byte-buffer that stores values in aodvTraCI byte-order
         */
        class aodvTraCIBuffer {
            public:
                aodvTraCIBuffer() : buf() {
                    buf_index = 0;
                }

                aodvTraCIBuffer(std::string buf) : buf(buf) {
                    buf_index = 0;
                }
                ...
                template<typename T> aodvTraCIBuffer& operator >>(T& out) {
                    out = read<T>();
                    return *this;
                }

                template<typename T> aodvTraCIBuffer& operator <<(const T& inv) {
                    write(inv);
                    return *this;
                }
        ...
        simtime_t connectAt; /**< when to connect to aodvTraCI server (must be the initial timestep of the server) */
        simtime_t firstStepAt; /**< when to start synchronizing with the aodvTraCI server (-1: immediately after connecting) */
        ...
        std::list<std::pair<aodvTraCICoord, aodvTraCICoord> > roiRects; /**< which rectangles (e.g. "0,0-10,10 20,20-30,30) are considered to consitute the region of interest, if not empty */

        void* socketPtr;
        aodvTraCICoord netbounds1; /* network boundaries as reported by aodvTraCI (x1, y1) */
        aodvTraCICoord netbounds2; /* network boundaries as reported by aodvTraCI (x2, y2) */
        ...
        uint32_t activeVehicleCount; /**< number of vehicles reported as active by aodvTraCI server */
        bool autoShutdownTriggered;
        cMessage* connectAndStartTrigger; /**< self-message scheduled for when to connect to aodvTraCI server and start running */
         ...
         */
        bool isInRegionOfInterest(const aodvTraCICoord& position, std::string road_id, double speed, double angle);

        /**
         * sends a single command via aodvTraCI, checks status response, returns additional responses
         */
        aodvTraCIBuffer queryaodvTraCI(uint8_t commandId, const aodvTraCIBuffer& buf = aodvTraCIBuffer());

        /**
         * sends a single command via aodvTraCI, expects no reply, returns true if successful
         */
        aodvTraCIScenarioManager::aodvTraCIBuffer queryaodvTraCIOptional(uint8_t commandId, const aodvTraCIBuffer& buf, bool& success, std::string* errorMsg = 0);

        /**
         * returns byte-buffer containing a aodvTraCI command with optional parameters
         */
        std::string makeaodvTraCICommand(uint8_t commandId, aodvTraCIBuffer buf = aodvTraCIBuffer());

        /**
         * sends a message via aodvTraCI (after adding the header)
         */
        void sendaodvTraCIMessage(std::string buf);

        /**
         * receives a message via aodvTraCI (and strips the header)
         */
        std::string receiveaodvTraCIMessage();

        /**
         * commonly employed technique to get string values via aodvTraCI
         */
        ...
        /**
         * convert aodvTraCI coordinates to OMNeT++ coordinates
         */
        Coord aodvTraCI2omnet(aodvTraCICoord coord) const;

        /**
         * convert OMNeT++ coordinates to aodvTraCI coordinates
         */
        aodvTraCICoord omnet2aodvTraCI(Coord coord) const;

        /**
         * convert aodvTraCI angle to OMNeT++ angle (in rad)
         */
        double aodvTraCI2omnetAngle(double angle) const;

        /**
         * convert OMNeT++ angle (in rad) to aodvTraCI angle
         */
        double omnet2aodvTraCIAngle(double angle) const;

        void subscribeToVehicleVariables(std::string vehicleId);
        void unsubscribeFromVehicleVariables(std::string vehicleId);
        void processSimSubscription(std::string objectId, aodvTraCIBuffer& buf);
        void processVehicleSubscription(std::string objectId, aodvTraCIBuffer& buf);
        void processSubcriptionResult(aodvTraCIBuffer& buf);
};

template<> void aodvTraCIScenarioManager::aodvTraCIBuffer::write(std::string inv);
template<> void aodvTraCIScenarioManager::aodvTraCIBuffer::write(aodvTraCIScenarioManager::aodvTraCICoord inv);
...
class aodvTraCIScenarioManagerAccess
{
    public:
        aodvTraCIScenarioManagerAccess() {
            o = dynamic_cast<aodvTraCIScenarioManager*>(simulation.getModuleByPath("manager"));
            if (!o) throw cRuntimeError("Could not find a aodvTraCIScenarioManager module named manager");
        };

        aodvTraCIScenarioManager* get() {
            return o;
        }

    protected:
        aodvTraCIScenarioManager* o;
