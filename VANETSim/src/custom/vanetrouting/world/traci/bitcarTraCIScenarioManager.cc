...
#include "world/traci/bitcarTraCIConstants.h"
#include "mobility/single/bitcarTraCIMobility.h"

Define_Module(bitcarTraCIScenarioManager);

bitcarTraCIScenarioManager::~bitcarTraCIScenarioManager()
{
    cancelAndDelete(connectAndStartTrigger);
    cancelAndDelete(executeOneTimestepTrigger);
}

void bitcarTraCIScenarioManager::initialize(int stage)
{
    ...
    while (std::getline(roiRects_i, rect, ' '))
    {
        ...
        rect_i >> y2;
        ASSERT(rect_i);
        roiRects.push_back(std::pair<bitcarTraCICoord, bitcarTraCICoord>(bitcarTraCICoord(x1, y1), bitcarTraCICoord(x2, y2)));
    }
    ...
    MYDEBUG << "initialized bitcarTraCIScenarioManager" << endl;
}

std::string bitcarTraCIScenarioManager::receivebitcarTraCIMessage()
{
    if (!socketPtr)
        error("Connection to bitcarTraCI server lost");
    ...
    uint32_t msgLength;
    {
        ...
        while (bytesRead < sizeof(uint32_t))
        {
            ...
            {
                error("Connection to bitcarTraCI server closed unexpectedly. Check your server's log");
            }
            else
            {
                if (sock_errno() == EINTR)
                ...
                if (sock_errno() == EAGAIN)
                    continue;
                error("Connection to bitcarTraCI server lost. Check your server's log. Error message: %d: %s", sock_errno(),
                        strerror(sock_errno()));
            }
        }
        bitcarTraCIBuffer(std::string(buf2, sizeof(uint32_t))) >> msgLength;
    }

    uint32_t bufLength = msgLength - sizeof(msgLength);
    char buf[bufLength];
    {
        MYDEBUG << "Reading bitcarTraCI message of " << bufLength << " bytes" << endl;
        uint32_t bytesRead = 0;
...
void bitcarTraCIScenarioManager::sendbitcarTraCIMessage(std::string buf)
{
    if (!socketPtr)
        error("Connection to bitcarTraCI server lost");

    {
        ...
        bitcarTraCIBuffer buf2 = bitcarTraCIBuffer();
        buf2 << msgLength;
    ...
    {
        MYDEBUG << "Writing bitcarTraCI message of " << buf.length() << " bytes" << endl;
        uint32_t bytesWritten = 0;
...
std::string bitcarTraCIScenarioManager::makebitcarTraCICommand(uint8_t commandId, bitcarTraCIBuffer buf)
{
    if (sizeof(uint8_t) + sizeof(uint8_t) + buf.str().length() > 0xFF)
    {
        ...
        return (bitcarTraCIBuffer() << static_cast<uint8_t>(0) << len << commandId).str() + buf.str();
    }
    uint8_t len = sizeof(uint8_t) + sizeof(uint8_t) + buf.str().length();
    return (bitcarTraCIBuffer() << len << commandId).str() + buf.str();
}

bitcarTraCIScenarioManager::bitcarTraCIBuffer bitcarTraCIScenarioManager::querybitcarTraCI(uint8_t commandId, const bitcarTraCIBuffer& buf)
{
    sendbitcarTraCIMessage(makebitcarTraCICommand(commandId, buf));

    bitcarTraCIBuffer obuf(receivebitcarTraCIMessage());
    uint8_t cmdLength;
    obuf >> cmdLength;
    uint8_t commandResp;
    obuf >> commandResp;
    ...
    if (result == RTYPE_NOTIMPLEMENTED)
        error("bitcarTraCI server reported command 0x%2x not implemented (\"%s\"). Might need newer version.", commandId,
                description.c_str());
    if (result == RTYPE_ERR)
        error("bitcarTraCI server reported error executing command 0x%2x (\"%s\").", commandId, description.c_str());
    ASSERT(result == RTYPE_OK);
    return obuf;
}

bitcarTraCIScenarioManager::bitcarTraCIBuffer bitcarTraCIScenarioManager::querybitcarTraCIOptional(uint8_t commandId, const bitcarTraCIBuffer& buf,
...
void bitcarTraCIScenarioManager::connect()
{
    MYDEBUG << "bitcarTraCIScenarioManager connecting to bitcarTraCI server" << endl;
    ...
    {
        error("Invalid bitcarTraCI server address: %s", host.c_str());
        return;
    }
    ...
    if (MYSOCKET < 0)
        error("Could not create socket to connect to bitcarTraCI server");

    if (::connect(MYSOCKET, (sockaddr const*) &address, sizeof(address)) < 0)
    {
        error(
                "Could not connect to bitcarTraCI server. Make sure it is running and not behind a firewall. Error message: %d: %s",
...
void bitcarTraCIScenarioManager::init_traci()
{
    {
        std::pair<uint32_t, std::string> version = bitcarTraCIScenarioManager::commandGetVersion();
        uint32_t apiVersion = version.first;
        std::string serverVersion = version.second;

        if ((apiVersion == 3) || (apiVersion == 5) || (apiVersion == 6) || (apiVersion == 7))
        {
            MYDEBUG << "bitcarTraCI server \"" << serverVersion << "\" reports API version " << apiVersion << endl;
        }
        else
        {
            error("bitcarTraCI server \"%s\" reports API version %d. This server is unsupported.", serverVersion.c_str(),
                    apiVersion);
        }
...

    {
        ...
        bitcarTraCIBuffer buf = querybitcarTraCI(CMD_GET_SIM_VARIABLE,
                bitcarTraCIBuffer() << static_cast<uint8_t>(VAR_NET_BOUNDING_BOX) << std::string("sim0"));
        ...
        netbounds1 = bitcarTraCICoord(x1, y1);
        netbounds2 = bitcarTraCICoord(x2, y2);
        MYDEBUG << "bitcarTraCI reports network boundaries (" << x1 << ", " << y1 << ")-(" << x2 << ", " << y2 << ")" << endl;
    }

    {
        ...
        bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SUBSCRIBE_SIM_VARIABLE,
                bitcarTraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2
...

    {
        ...
        bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SUBSCRIBE_VEHICLE_VARIABLE,
                bitcarTraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1);
        processSubcriptionResult(buf);
        ASSERT(buf.eof());
    }
...
void bitcarTraCIScenarioManager::finish()
{
...
void bitcarTraCIScenarioManager::handleMessage(cMessage *msg)
{
    ...
    error("bitcarTraCIScenarioManager doesn't handle messages from other modules");
}

void bitcarTraCIScenarioManager::handleSelfMsg(cMessage *msg)
{
    ...
    error("bitcarTraCIScenarioManager received unknown self-message");
}

std::pair<uint32_t, std::string> bitcarTraCIScenarioManager::commandGetVersion()
{
    bool success = false;
    bitcarTraCIBuffer buf = querybitcarTraCIOptional(CMD_GETVERSION, bitcarTraCIBuffer(), success);
...
void bitcarTraCIScenarioManager::commandSetSpeedMode(std::string nodeId, int32_t bitset)
{
    uint8_t variableId = VAR_SPEEDSETMODE;
    uint8_t variableType = TYPE_INTEGER;
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << variableId << nodeId << variableType << bitset);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::commandSetSpeed(std::string nodeId, double speed)
{
    uint8_t variableId = VAR_SPEED;
    uint8_t variableType = TYPE_DOUBLE;
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << variableId << nodeId << variableType << speed);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::commandNewRoute(std::string nodeId, std::string roadId)
{
    uint8_t variableId = LANE_EDGE_ID;
    uint8_t variableType = TYPE_STRING;
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << variableId << nodeId << variableType << roadId);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::commandSetVehicleParking(std::string nodeId)
{
    uint8_t variableId = REMOVE;
    uint8_t variableType = TYPE_BYTE;
    uint8_t value = REMOVE_PARKING;
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << variableId << nodeId << variableType << value);
    ASSERT(buf.eof());
}

std::string bitcarTraCIScenarioManager::commandGetEdgeId(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_ROAD_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string bitcarTraCIScenarioManager::commandGetCurrentEdgeOnRoute(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, LANE_EDGE_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string bitcarTraCIScenarioManager::commandGetLaneId(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_LANE_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

double bitcarTraCIScenarioManager::commandGetLanePosition(std::string nodeId)
{
    return genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_LANEPOSITION,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::list<std::string> bitcarTraCIScenarioManager::commandGetPlannedEdgeIds(std::string nodeId)
{
    return genericGetStringList(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_EDGES,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string bitcarTraCIScenarioManager::commandGetRouteId(std::string nodeId)
{
    return genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_ROUTE_ID,
    RESPONSE_GET_VEHICLE_VARIABLE);
}

std::list<std::string> bitcarTraCIScenarioManager::commandGetRouteEdgeIds(std::string routeId)
{
    return genericGetStringList(CMD_GET_ROUTE_VARIABLE, routeId, VAR_EDGES,
    RESPONSE_GET_ROUTE_VARIABLE);
}

void bitcarTraCIScenarioManager::commandChangeRoute(std::string nodeId, std::string roadId, double travelTime)
{
    if (travelTime >= 0)
    {
        ...
        bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
                bitcarTraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId << newTimeT
                        << newTime);
        ASSERT(buf.eof());
    }
    else
    {
        ...
        std::string edgeId = roadId;
        bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
                bitcarTraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId);
        ASSERT(buf.eof());
    }
    {
        ...
        int32_t count = 0;
        bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
                bitcarTraCIBuffer() << variableId << nodeId << variableType << count);
        ASSERT(buf.eof());
    }
}

double bitcarTraCIScenarioManager::commandDistanceRequest(Coord position1, Coord position2, bool returnDrivingDistance)
{
    uint8_t variable = DISTANCE_REQUEST;
    std::string simId = "sim0";
    uint8_t variableType = TYPE_COMPOUND;
    int32_t count = 3;
    bitcarTraCICoord p1 = omnet2bitcarTraCI(position1);
    bitcarTraCICoord p2 = omnet2bitcarTraCI(position2);
    uint8_t dType = static_cast<uint8_t>(returnDrivingDistance ? REQUEST_DRIVINGDIST : REQUEST_AIRDIST);

    // query road network boundaries
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_GET_SIM_VARIABLE,
            bitcarTraCIBuffer() << variable << simId << variableType << count << p1 << p2 << dType);
    uint8_t cmdLength_resp;
    buf >> cmdLength_resp;
    uint8_t commandId_resp;
    buf >> commandId_resp;
...
void bitcarTraCIScenarioManager::commandStopNode(std::string nodeId, std::string roadId, double pos, uint8_t laneid,
        double radius, double waittime)
{
    ...
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId << stopPosT << stopPos
                    << stopLaneT << stopLane << durationT << duration);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::commandSetTrafficLightProgram(std::string trafficLightId, std::string program)
{
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_TL_VARIABLE,
            bitcarTraCIBuffer() << static_cast<uint8_t>(TL_PROGRAM) << trafficLightId << static_cast<uint8_t>(TYPE_STRING)
                    << program);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::commandSetTrafficLightPhaseIndex(std::string trafficLightId, int32_t index)
{
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_TL_VARIABLE,
            bitcarTraCIBuffer() << static_cast<uint8_t>(TL_PHASE_INDEX) << trafficLightId
                    << static_cast<uint8_t>(TYPE_INTEGER) << index);
    ASSERT(buf.eof());
}

std::list<std::string> bitcarTraCIScenarioManager::commandGetPolygonIds()
{
    return genericGetStringList(CMD_GET_POLYGON_VARIABLE, "", ID_LIST, RESPONSE_GET_POLYGON_VARIABLE);
}

std::string bitcarTraCIScenarioManager::commandGetPolygonTypeId(std::string polyId)
{
    return genericGetString(CMD_GET_POLYGON_VARIABLE, polyId, VAR_TYPE, RESPONSE_GET_POLYGON_VARIABLE);
}

std::list<Coord> bitcarTraCIScenarioManager::commandGetPolygonShape(std::string polyId)
{
    return genericGetCoordList(CMD_GET_POLYGON_VARIABLE, polyId, VAR_SHAPE, RESPONSE_GET_POLYGON_VARIABLE);
}

void bitcarTraCIScenarioManager::commandSetPolygonShape(std::string polyId, std::list<Coord> points)
{
    ...
    {
        bitcarTraCICoord pos = omnet2bitcarTraCI(*i);
        buf << static_cast<double>(pos.x) << static_cast<double>(pos.y);
    }
    bitcarTraCIBuffer obuf = querybitcarTraCI(CMD_SET_POLYGON_VARIABLE, buf);
    ASSERT(obuf.eof());
}

void bitcarTraCIScenarioManager::commandAddPolygon(std::string polyId, std::string polyType, const vanetTraCIColor& color,
        bool filled, int32_t layer, std::list<Coord> points)
{
    ...
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_POLYGON_VARIABLE, p);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::commandRemovePolygon(std::string polyId, int32_t layer)
{
    bitcarTraCIBuffer p;
...
void bitcarTraCIScenarioManager::commandAddPoi(std::string poiId, std::string poiType, const vanetTraCIColor& color, int32_t layer,
        Coord pos)
{
    bitcarTraCIBuffer p;

    bitcarTraCICoord bitcarTraCIPos = omnet2bitcarTraCI(pos);
    ...
    p << static_cast<uint8_t>(TYPE_COLOR) << color.red << color.green << color.blue << color.alpha;
    p << static_cast<uint8_t>(TYPE_INTEGER) << layer;
    p << static_cast<uint8_t>(POSITION_2D) << bitcarTraCIPos.x << bitcarTraCIPos.y;

    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SET_POI_VARIABLE, p);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::commandRemovePoi(std::string poiId, int32_t layer)
{
    bitcarTraCIBuffer p;
        ...
        simtime_t emitTime_st, double emitPosition, double emitSpeed, int8_t emitLane)
{
    ...
    bitcarTraCIBuffer buf = querybitcarTraCIOptional(CMD_SET_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << variableId << vehicleId << variableType << count << (uint8_t) TYPE_STRING << vehicleTypeId
...
void bitcarTraCIScenarioManager::addModule(std::string nodeId, std::string type, std::string name, std::string displayString,
        const Coord& position, std::string road_id, double speed, double acceleration, double angle)
{
    ...
    // pre-initialize bitcarTraCIMobility
    for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++)
    {
        cModule* submod = iter();
        bitcarTraCIMobility* mm = dynamic_cast<bitcarTraCIMobility*>(submod);
        if (!mm)
            continue;
    ...
    // post-initialize bitcarTraCIMobility
    for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++)
    {
        cModule* submod = iter();
        bitcarTraCIMobility* mm = dynamic_cast<bitcarTraCIMobility*>(submod);
...
cModule* bitcarTraCIScenarioManager::getManagedModule(std::string nodeId)
{
    if (hosts.find(nodeId) == hosts.end())
        return 0;
    return hosts[nodeId];
...
bool bitcarTraCIScenarioManager::isModuleUnequipped(std::string nodeId)
{
...
void bitcarTraCIScenarioManager::deleteModule(std::string nodeId)
{
...
bool bitcarTraCIScenarioManager::isInRegionOfInterest(const bitcarTraCICoord& position, std::string road_id, double speed,
        double angle)
{
    ...
    if (roiRects.size() > 0)
    {
        for (std::list<std::pair<bitcarTraCICoord, bitcarTraCICoord> >::const_iterator i = roiRects.begin(); i != roiRects.end();
                ++i)
        {
...
uint32_t bitcarTraCIScenarioManager::getCurrentTimeMs()
{
    return static_cast<uint32_t>(round(simTime().dbl() * 1000));
}

void bitcarTraCIScenarioManager::executeOneTimestep()
{

    MYDEBUG << "Triggering bitcarTraCI server simulation advance to t=" << simTime() << endl;

    uint32_t targetTime = getCurrentTimeMs();

    if (targetTime > round(connectAt.dbl() * 1000))
    {
        bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SIMSTEP2, bitcarTraCIBuffer() << targetTime);
...
std::string bitcarTraCIScenarioManager::genericGetString(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
    ...
    bitcarTraCIBuffer buf = querybitcarTraCI(commandId, bitcarTraCIBuffer() << variableId << objectId);
...
Coord bitcarTraCIScenarioManager::genericGetCoord(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
    ...
    return bitcarTraCI2omnet(bitcarTraCICoord(x, y));
}

double bitcarTraCIScenarioManager::genericGetDouble(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
...
std::list<std::string> bitcarTraCIScenarioManager::genericGetStringList(uint8_t commandId, std::string objectId,
...
std::list<Coord> bitcarTraCIScenarioManager::genericGetCoordList(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        double y;
        buf >> y;
        Coord pos = bitcarTraCI2omnet(bitcarTraCICoord(x, y));
        res.push_back(pos);
    }
...
Coord bitcarTraCIScenarioManager::bitcarTraCI2omnet(bitcarTraCICoord coord) const
{
    return Coord(coord.x - netbounds1.x + margin, (netbounds2.y - netbounds1.y) - (coord.y - netbounds1.y) + margin);
}

bitcarTraCIScenarioManager::bitcarTraCICoord bitcarTraCIScenarioManager::omnet2bitcarTraCI(Coord coord) const
{
    return bitcarTraCICoord(coord.x + netbounds1.x - margin,
            (netbounds2.y - netbounds1.y) - (coord.y - netbounds1.y) + margin);
}

double bitcarTraCIScenarioManager::bitcarTraCI2omnetAngle(double angle) const
{

    // rotate angle so 0 is east (in bitcarTraCI's angle interpretation 0 is south)
    angle = angle - 90;
...
double bitcarTraCIScenarioManager::omnet2bitcarTraCIAngle(double angle) const
{
...
void bitcarTraCIScenarioManager::subscribeToVehicleVariables(std::string vehicleId)
{
    ...
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SUBSCRIBE_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2 << variable3
                    << variable4 << variable5);
    processSubcriptionResult(buf);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::unsubscribeFromVehicleVariables(std::string vehicleId)
{
    // subscribe to some attributes of the vehicle
    uint32_t beginTime = 0;
    uint32_t endTime = 0x7FFFFFFF;
    ...
    bitcarTraCIBuffer buf = querybitcarTraCI(CMD_SUBSCRIBE_VEHICLE_VARIABLE,
            bitcarTraCIBuffer() << beginTime << endTime << objectId << variableNumber);
    ASSERT(buf.eof());
}

void bitcarTraCIScenarioManager::processSimSubscription(std::string objectId, bitcarTraCIBuffer& buf)
{
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            if (isokay == RTYPE_NOTIMPLEMENTED)
                error(
                        "bitcarTraCI server reported subscribing to variable 0x%2x not implemented (\"%s\"). Might need newer version.",
                        variable1_resp, description.c_str());
            error("bitcarTraCI server reported error subscribing to variable 0x%2x (\"%s\").", variable1_resp,
                    description.c_str());
        }
        ...
        if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "bitcarTraCI reports " << count << " departed vehicles." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_ARRIVED_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "bitcarTraCI reports " << count << " arrived vehicles." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_TELEPORT_STARTING_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "bitcarTraCI reports " << count << " vehicles starting to teleport." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_TELEPORT_ENDING_VEHICLES_IDS)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "bitcarTraCI reports " << count << " vehicles ending teleport." << endl;
            for (uint32_t i = 0; i < count; ++i)
            {
                std::string idstring;
                buf >> idstring;
        ...
        else if (variable1_resp == VAR_TIME_STEP)
        {
            ...
            buf >> serverTimestep;
            MYDEBUG << "bitcarTraCI reports current time step as " << serverTimestep << "ms." << endl;
...
void bitcarTraCIScenarioManager::processVehicleSubscription(std::string objectId, bitcarTraCIBuffer& buf)
{
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            if (isSubscribed)
            {
                if (isokay == RTYPE_NOTIMPLEMENTED)
                    error(
                            "bitcarTraCI server reported subscribing to vehicle variable 0x%2x not implemented (\"%s\"). Might need newer version.",
                            variable1_resp, errormsg.c_str());
                error("bitcarTraCI server reported error subscribing to vehicle variable 0x%2x (\"%s\").", variable1_resp,
                        errormsg.c_str());
            }
        }
        else if (variable1_resp == ID_LIST)
        {
            ...
            uint32_t count;
            buf >> count;
            MYDEBUG << "bitcarTraCI reports " << count << " active vehicles." << endl;
    ...
    Coord p = bitcarTraCI2omnet(bitcarTraCICoord(px, py));
    if ((p.x < 0) || (p.y < 0))
        error("received bad node position (%.2f, %.2f), translated to (%.2f, %.2f)", px, py, p.x, p.y);

    double angle = bitcarTraCI2omnetAngle(angle_bitcarTraCI);

    cModule* mod = getManagedModule(objectId);

    // is it in the ROI?
    bool inRoi = isInRegionOfInterest(bitcarTraCICoord(px, py), edge, speed, angle);
    if (!inRoi)
    {
        if (mod)
        {
...
void bitcarTraCIScenarioManager::processSubcriptionResult(bitcarTraCIBuffer& buf)
{
    uint8_t cmdLength_resp;
    buf >> cmdLength_resp;
...
template<> void bitcarTraCIScenarioManager::bitcarTraCIBuffer::write(bitcarTraCICoord inv)
{
