...
Define_Module(bitcarTraCIMobility);
...
void bitcarTraCIMobility::Statistics::initialize()
{
...
void bitcarTraCIMobility::Statistics::watch(cSimpleModule& )
{
    WATCH(totalTime);
    WATCH(minSpeed);
    WATCH(maxSpeed);
...
void bitcarTraCIMobility::Statistics::recordScalars(cSimpleModule& module)
{
...
void bitcarTraCIMobility::initialize(int stage)
{
...
void bitcarTraCIMobility::setInitialPosition() {
    ASSERT(isPreInitialized);
    isPreInitialized = false;
}

void bitcarTraCIMobility::finish()
{
...
void bitcarTraCIMobility::handleSelfMessage(cMessage *msg)
{
...
void bitcarTraCIMobility::preInitialize(std::string external_id, const Coord& position, std::string road_id, double speed, double acceleration, double angle)
{
...
void bitcarTraCIMobility::nextPosition(const Coord& position, std::string road_id, double speed, double acceleration, double angle, bitcarTraCIScenarioManager::VehicleSignal signals)
{
...
void bitcarTraCIMobility::updateDisplayString() {
    ASSERT(-M_PI <= angle);
    ASSERT(angle < M_PI);
...
void bitcarTraCIMobility::fixIfHostGetsOutside()
{
    raiseErrorIfOutside();
}

double bitcarTraCIMobility::calculateCO2emission(double v, double a) const {
