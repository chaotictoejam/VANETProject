...
static void nl_kaodvvanet_callback(int sock);
...
void nl_init(void)
{
	...
	aodvnl.local.nl_groups = AODVVANETGRP_NOTIFY;
	aodvnl.local.nl_pid = getpid();

	/* This is the AODVVANET specific socket to communicate with the
	   AODVVANET kernel module */
	aodvnl.sock = socket(PF_NETLINK, SOCK_RAW, NETLINK_AODVVANET);

	if (aodvnl.sock < 0) {
		perror("Unable to create AODVVANET netlink socket");
		exit(-1);
	}
	...
	if (status == -1) {
		perror("Bind for AODVVANET netlink socket failed");
		exit(-1);
	}
	...
	if (attach_callback_func(aodvnl.sock, nl_kaodvvanet_callback) < 0) {
...
static void nl_kaodvvanet_callback(int sock)
{
	...
	case NLMSG_ERROR:
		...
		/* 	DEBUG(LOG_DEBUG, 0, "NLMSG_ACK"); */
			...
			DEBUG(LOG_DEBUG, 0, "NLMSG_ERROR, error=%d type=%s",
			      nlmerr->error, 
			      kaodvvanet_msg_type_to_str(nlmerr->msg.nlmsg_type));
		}
		break;

	case KAODVVANETM_DEBUG:
		DEBUG(LOG_DEBUG, 0, "kaodv: %s", NLMSG_DATA(nlm));
		break;
       	case KAODVVANETM_TIMEOUT:
		m = NLMSG_DATA(nlm);
	...
	case KAODVVANETM_ROUTE_UPDATE:
		...
		if (dest_addr.s_addr == AODVVANET_BROADCAST ||
		    dest_addr.s_addr ==
			  ...
			  int ifindex)
{
	...
	areq.n.nlmsg_len = NLMSG_LENGTH(sizeof(struct kaodvvanet_rt_msg));
	areq.n.nlmsg_type = KAODVVANETM_ADDROUTE;
	...
	if (rt_flags & RT_INET_DEST) {
		areq.m.flags |= KAODVVANET_RT_GW_ENCAP;
	}

	if (rt_flags & RT_REPAIR)
		areq.m.flags |= KAODVVANET_RT_REPAIR;
...
int nl_send_no_route_found_msg(struct in_addr dest)
{
	...
	areq.n.nlmsg_len = NLMSG_LENGTH(sizeof(struct kaodvvanet_rt_msg));
	areq.n.nlmsg_type = KAODVVANETM_NOROUTE_FOUND;
	areq.n.nlmsg_flags = NLM_F_REQUEST;
...
int nl_send_del_route_msg(struct in_addr dest, struct in_addr next_hop, int metric)
{
	...
	areq.n.nlmsg_len = NLMSG_LENGTH(sizeof(struct kaodvvanet_rt_msg));
	areq.n.nlmsg_type = KAODVVANETM_DELROUTE;
	areq.n.nlmsg_flags = NLM_F_REQUEST;
...
int nl_send_conf_msg(void)
{
	struct {
		struct nlmsghdr n;
		kaodvvanet_conf_msg_t cm;
	} areq;

	memset(&areq, 0, sizeof(areq));

	areq.n.nlmsg_len = NLMSG_LENGTH(sizeof(kaodvvanet_conf_msg_t));
	areq.n.nlmsg_type = KAODVVANETM_CONFIG;
	areq.n.nlmsg_flags = NLM_F_REQUEST;
