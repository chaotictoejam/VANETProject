...
#include <set>
#include "inetveins/common/stlutils.h"
#include "inetveins/common/ModuleAccess.h"
#include "inetveins/common/XMLUtils.h"
...
#include "inetveins/physicallayer/common/packetlevel/Interference.h"
#endif
...
NetworkConfiguratorBase::Matcher::Matcher(const char *pattern)
{
    ...
    while (tokenizer.hasMoreTokens())
        matchers.push_back(new inetveins::PatternMatcher(tokenizer.nextToken(), true, true, true));
}
...
NetworkConfiguratorBase::InterfaceMatcher::InterfaceMatcher(const char *pattern)
{
    ...
    while (tokenizer.hasMoreTokens()) {
        ...
        if (*token == '>')
            towardsMatchers.push_back(new inetveins::PatternMatcher(token + 1, true, true, true));
        else
            nameMatchers.push_back(new inetveins::PatternMatcher(token, true, true, true));
    }
}
