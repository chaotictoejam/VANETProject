...
#include "inetveins/networklayer/internetcloud/MatrixCloudDelayer.h"
...
MatrixCloudDelayer::Matcher::Matcher(const char *pattern)
{
    ...
    while (tokenizer.hasMoreTokens()) {
        ...
        matchers.push_back(new inetveins::PatternMatcher(token, true, true, true));
        if (*token != '*') {
            // add "*.token" too
            std::string subtoken("*.");
            subtoken += token;
            matchers.push_back(new inetveins::PatternMatcher(subtoken.c_str(), true, true, true));
        }
    }
}
