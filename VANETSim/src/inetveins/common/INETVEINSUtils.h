 ...
 */
INETVEINS_API std::string ltostr(long i);    //XXX make an ultostr as well, to be consistent with atoul

/**
 *  Converts a double to string
 */
INETVEINS_API std::string dtostr(double d);

/**
 *  Converts string to double
 */
INETVEINS_API double atod(const char *s);

/**
 *  Converts string to unsigned long
 */
INETVEINS_API unsigned long atoul(const char *s);

/**
 * Removes non-alphanumeric characters from the given string.
 */
INETVEINS_API std::string stripnonalnum(const char *s);

/**
 * Accepts a printf-like argument list, and returns the result in a string.
 * The limit is 1024 chars.
 */
INETVEINS_API std::string stringf(const char *fmt, ...);

/**
 * Accepts a vprintf-like argument list, and returns the result in a string.
 * The limit is 1024 chars.
 */
INETVEINS_API std::string vstringf(const char *fmt, va_list& args);
 ...
 */
INETVEINS_API cObject *createOneIfClassIsKnown(const char *className, const char *defaultNamespace = getSimulation()->getContext()->getClassName());

/**
 * Like cObjectFactory::createOne(), except it starts searching for the class in the given namespace
 */
INETVEINS_API cObject *createOne(const char *className, const char *defaultNamespace = getSimulation()->getContext()->getClassName());
