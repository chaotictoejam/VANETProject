...
SCTPAssociation *SCTPAssociation::cloneAssociation()
{
    ...
    assoc->transmissionQ = check_and_cast<SCTPQueue *>(inetveins::utils::createOne(queueClass));
    assoc->retransmissionQ = check_and_cast<SCTPQueue *>(inetveins::utils::createOne(queueClass));

    const char *sctpAlgorithmClass = sctpAlgorithm->getClassName();
    assoc->sctpAlgorithm = check_and_cast<SCTPAlgorithm *>(inetveins::utils::createOne(sctpAlgorithmClass));
...
void SCTPAssociation::initAssociation(SCTPOpenCommand *openCmd)
{
    ...
    transmissionQ = check_and_cast<SCTPQueue *>(inetveins::utils::createOne(queueClass));

    retransmissionQ = check_and_cast<SCTPQueue *>(inetveins::utils::createOne(queueClass));
    ...
    sctpAlgorithm = check_and_cast<SCTPAlgorithm *>(inetveins::utils::createOne(sctpAlgorithmClass));
...
void SCTPAssociation::sendInit()
{
    ...
    if (remoteAddressList.size() > 0) {
        for (auto it = remoteAddressList.begin(); it != remoteAddressList.end(); it++) {
...
SCTPForwardTsnChunk *SCTPAssociation::createForwardTsnChunk(const L3Address& pid)
{
    ...
    for (auto it = retransmissionQ->payloadQueue.begin(); it != retransmissionQ->payloadQueue.end(); it++) {
        chunk = it->second;
    ...
    int32 i = 0;
    for (auto j = sidMap.begin(); j != sidMap.end(); j++) {
...
bool SCTPAssociation::makeRoomForTsn(const uint32 tsn, const uint32 length, const bool uBit)
{
           ...
           (tryTSN > state->gapList.getCumAckTSN()))
    {
        ...
        for (auto iterator = receiveStreams.begin();
             iterator != receiveStreams.end(); iterator++)
        {
...
SCTPDataVariables *SCTPAssociation::peekAbandonedChunk(const SCTPPathVariables *path)
{
    ...
    if (state->prMethod != 0 && !retransmissionQ->payloadQueue.empty()) {
        for (auto it = retransmissionQ->payloadQueue.begin();
