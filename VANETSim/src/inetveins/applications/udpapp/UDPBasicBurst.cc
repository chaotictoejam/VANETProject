...
#include "inetveins/applications/udpapp/UDPBasicBurst.h"
...
EXECUTE_ON_STARTUP(
        cEnum * e = cEnum::find("inetveins::ChooseDestAddrMode");
        if (!e)
            enums.getInstance()->add(e = new cEnum("inetveins::ChooseDestAddrMode"));
...
void UDPBasicBurst::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        int addrMode = cEnum::get("inetveins::ChooseDestAddrMode")->lookup(addrModeStr);
        if (addrMode == -1)
