...
#include "world/traci/aodvTraCIScenarioManagerLaunchd.h"
...
Define_Module(aodvTraCIScenarioManagerLaunchd);

aodvTraCIScenarioManagerLaunchd::~aodvTraCIScenarioManagerLaunchd()
{
}


void aodvTraCIScenarioManagerLaunchd::initialize(int stage)
{
    if (stage != 1) {
        aodvTraCIScenarioManager::initialize(stage);
        return;
    }
...
void aodvTraCIScenarioManagerLaunchd::finish()
{
    aodvTraCIScenarioManager::finish();
}

void aodvTraCIScenarioManagerLaunchd::init_traci() {
    {
        ...
        MYDEBUG << "aodvTraCI launchd reports version \"" << serverVersion << "\"" << endl;
    }
    ...
    sendaodvTraCIMessage(makeaodvTraCICommand(CMD_FILE_SEND, buf));
    ...
    if (result != RTYPE_OK) {
        EV << "Warning: Received non-OK response from aodvTraCI server to command " << CMD_FILE_SEND << ":" << description.c_str() << std::endl;
    }

    aodvTraCIScenarioManager::init_traci();
}
