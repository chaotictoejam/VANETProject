...
#include "applications/traci/bitcarTraCITestApp.h"
#include "VanetModuleAccess.h"
#include "NodeStatus.h"
#include <cmath>

Define_Module(bitcarTraCITestApp);

simsignal_t bitcarTraCITestApp::mobilityStateChangedSignal = registerSignal("mobilityStateChanged");

void bitcarTraCITestApp::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        EV_DEBUG << "bitcarTraCITestApp initialized with testNumber=" << testNumber << std::endl;
    }
    else if (stage == 3)
    {
        bool isOperational;
...
void bitcarTraCITestApp::finish() {
}

void bitcarTraCITestApp::handleSelfMsg(cMessage *msg) {
}

void bitcarTraCITestApp::handleLowerMsg(cMessage* msg) {
    delete msg;
}

void bitcarTraCITestApp::handleMessage(cMessage* msg) {
...
void bitcarTraCITestApp::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj) {
...
void bitcarTraCITestApp::handlePositionUpdate() {
    ...
    if (testNumber == testCounter++) {
        ...
        if (t == 31) {
            ...
            bitcarTraCIMobility* traci2 = check_and_cast<bitcarTraCIMobility*>(findModuleWhereverInNode("mobility", mod));
