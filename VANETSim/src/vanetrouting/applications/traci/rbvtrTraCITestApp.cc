...
#include "applications/traci/rbvtrTraCITestApp.h"
#include "VanetModuleAccess.h"
#include "NodeStatus.h"
#include <cmath>

Define_Module(rbvtrTraCITestApp);

simsignal_t rbvtrTraCITestApp::mobilityStateChangedSignal = registerSignal("mobilityStateChanged");

void rbvtrTraCITestApp::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        EV_DEBUG << "rbvtrTraCITestApp initialized with testNumber=" << testNumber << std::endl;
    }
    else if (stage == 3)
    {
        bool isOperational;
...
void rbvtrTraCITestApp::finish() {
}

void rbvtrTraCITestApp::handleSelfMsg(cMessage *msg) {
}

void rbvtrTraCITestApp::handleLowerMsg(cMessage* msg) {
    delete msg;
}

void rbvtrTraCITestApp::handleMessage(cMessage* msg) {
...
void rbvtrTraCITestApp::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj) {
...
void rbvtrTraCITestApp::handlePositionUpdate() {
    ...
    if (testNumber == testCounter++) {
        ...
        if (t == 31) {
            ...
            rbvtrTraCIMobility* traci2 = check_and_cast<rbvtrTraCIMobility*>(findModuleWhereverInNode("mobility", mod));
