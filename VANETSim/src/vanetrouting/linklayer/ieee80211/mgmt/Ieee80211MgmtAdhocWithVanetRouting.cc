...
#include "Ieee80211MgmtAdhocWithVanetRouting.h"
#include "Ieee802Ctrl_m.h"
#include "ControlVanetRouting_m.h"


Define_Module(Ieee80211MgmtAdhocWithVanetRouting);


void Ieee80211MgmtAdhocWithVanetRouting::startRouting()
{
    cModuleType *moduleType;
    cModule *module;
    moduleType = cModuleType::find(par("routingProtocol").stringValue());
    if (moduleType == NULL)
        opp_error("Ieee80211MgmtAdhocWithVanetRouting:: Routing protocol not found %s",par("routingProtocol").stringValue());
    module = moduleType->create("VanetRoutingProtocol", this);
    routingModule = dynamic_cast <VanetRoutingBase*> (module);
...
void Ieee80211MgmtAdhocWithVanetRouting::initialize(int stage)
{
...
void Ieee80211MgmtAdhocWithVanetRouting::handleTimer(cMessage *msg)
{
    ASSERT(false);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleUpperMessage(cPacket *msg)
{
    Ieee80211DataFrame *frame = encapsulate(msg);
    sendOrEnqueue(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleCommand(int msgkind, cObject *ctrl)
{
    error("handleCommand(): no commands supported");
}


void Ieee80211MgmtAdhocWithVanetRouting::receiveChangeNotification(int category, const cObject *details)
{
    Enter_Method_Silent();
    //printNotificationBanner(category, details);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleAuthenticationFrame(Ieee80211AuthenticationFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleDeauthenticationFrame(Ieee80211DeauthenticationFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleAssociationRequestFrame(Ieee80211AssociationRequestFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleAssociationResponseFrame(Ieee80211AssociationResponseFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleReassociationRequestFrame(Ieee80211ReassociationRequestFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleReassociationResponseFrame(Ieee80211ReassociationResponseFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleDisassociationFrame(Ieee80211DisassociationFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleBeaconFrame(Ieee80211BeaconFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleProbeRequestFrame(Ieee80211ProbeRequestFrame *frame)
{
    dropManagementFrame(frame);
}

void Ieee80211MgmtAdhocWithVanetRouting::handleProbeResponseFrame(Ieee80211ProbeResponseFrame *frame)
{
    dropManagementFrame(frame);
}


void Ieee80211MgmtAdhocWithVanetRouting::actualizeReactive(cPacket *pkt,bool out)
{
    VanetAddress dest,next;
...
cPacket *Ieee80211MgmtAdhocWithVanetRouting::decapsulate(Ieee80211DataFrame *frame)
{
...
void Ieee80211MgmtAdhocWithVanetRouting::handleMessage(cMessage *msg)
{
...
void Ieee80211MgmtAdhocWithVanetRouting::handleDataFrame(Ieee80211DataFrame *frame)
{
...
bool Ieee80211MgmtAdhocWithVanetRouting::forwardMessage (Ieee80211DataFrame *frame)
{
...
bool Ieee80211MgmtAdhocWithVanetRouting::macLabelBasedSend(Ieee80211DataFrame *frame)
{
    ...
    VanetAddress dest = VanetAddress(frame->getAddress4());
    VanetAddress src = VanetAddress(frame->getAddress3());
    Ieee80211MeshFrame *frame2  = dynamic_cast<Ieee80211MeshFrame *>(frame);
    ...
    else
    {
        ...
        {
            ControlVanetRouting *ctrlvanet = new ControlVanetRouting();
            ctrlvanet->setOptionCode(VANET_ROUTE_NOROUTE);
            ctrlvanet->setDestAddress(dest);
            //  ctrlvanet->setSrcAddress(myAddress);
            ctrlvanet->setSrcAddress(src);
            ctrlvanet->encapsulate(frame);
            frame = NULL;
            send(ctrlvanet,"routingOut");
        }
    }
...
void Ieee80211MgmtAdhocWithVanetRouting::handleRoutingMessage(cPacket *msg)
{
...
Ieee80211DataFrame *Ieee80211MgmtAdhocWithVanetRouting::encapsulate(cPacket *msg)
{
    if (!routingModule)
    {
    ...
    //
    VanetAddress nextHop;
    int iface;
    double cost;
    if (!routingModule->getNextHop(VanetAddress(dest),nextHop,iface,cost)) //send the packet to the routingMo
    {
        if (!routingModule->isProactive())
        {
            ControlVanetRouting *ctrlvanet = new ControlVanetRouting();
            ctrlvanet->setOptionCode(VANET_ROUTE_NOROUTE);
            ctrlvanet->setDestAddress(VanetAddress(dest));
            ctrlvanet->setSrcAddress(VanetAddress(myAddress));
            ctrlvanet->encapsulate(frame);
            send(ctrlvanet,"routingOut");
            return NULL;
        }
...
Ieee80211DataFrame *Ieee80211MgmtAdhocWithVanetRouting::encapsulate(cPacket *msg,MACAddress dest)
{
