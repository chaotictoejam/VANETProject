...
#ifndef NS_PORT
static AVLIST(rreq_records);
static AVLIST(rreq_blacklist);
                                ...
                                struct ip_data *ipd)
{
    ...
    if (avseek_list_find(rt->dest_addr))
        return;
    ...
    /* Remember that we are seeking this destination and setup the
       timers */
    seek_entry = avseek_list_insert(rt->dest_addr, rt->dest_seqno,
                                  ttl, flags, ipd);
        ...
        u_int32_t rreq_id)
{
    ...
    avlist_foreach(pos, &rreq_records)
    {
...
struct blacklist *NS_CLASS rreq_blackavlist_insert(struct in_addr dest_addr)
{

    struct blacklist *bl;

    /* First check if this rreq packet is already buffered */
    bl = rreq_blackavlist_find(dest_addr);
    ...
    timer_init(&bl->bl_timer, &NS_CLASS rreq_blackavlist_timeout, bl);

    avlist_add(&rreq_blacklist, &bl->l);

    timer_set_timeout(&bl->bl_timer, BLACKAVLIST_TIMEOUT);
    return bl;
}


struct blacklist *NS_CLASS rreq_blackavlist_find(struct in_addr dest_addr)
{
    avlist_t *pos;

    avlist_foreach(pos, &rreq_blacklist)
    {
...
void NS_CLASS rreq_blackavlist_timeout(void *arg)
{
