 ...
 *
 * For more information on the routing algorithm, see the VS_GPSR paper
...
// KLUDGE: the VS_GPSR packet is now used to wrap the content of network datagrams
// KLUDGE: we should rather add these fields as header extensions
class INET_API VS_GPSR : public cSimpleModule, public ILifecycle, public INotifiable, public INetfilter::IHook
{
    private:
        // VS_GPSR parameters
        VS_GPSRPlanarizationMode planarizationMode;
        ...
        static VS_PositionTable globalPositionTable; // KLUDGE: implement position registry protocol

        // internal
        cMessage * beaconTimer;
        cMessage * purgeNeighborsTimer;
        VS_PositionTable neighborPositionTable;
    ...
    private:
        ...
        // handling beacons
        VS_GPSRBeacon * createBeacon();
        void sendBeacon(VS_GPSRBeacon * beacon, double delay);
        void processBeacon(VS_GPSRBeacon * beacon);

        // handling packets
        VS_GPSRPacket * createPacket(IPvXAddress destination, cPacket * content);
        int computePacketBitLength(VS_GPSRPacket * packet);
