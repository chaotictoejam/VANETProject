...
std::vector<IPvXAddress>VS_PositionTable::getAddresses() const {
...
bool VS_PositionTable::hasPosition(const IPvXAddress & address) const {
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
    return it != addressToPositionMap.end();
}

Coord VS_PositionTable::getPosition(const IPvXAddress & address) const {
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
...
void VS_PositionTable::setPosition(const IPvXAddress & address, const Coord & coord) {
    ASSERT(!address.isUnspecified());
    addressToPositionMap[address] = AddressToPositionMapValue(simTime(), coord);
}

void VS_PositionTable::removePosition(const IPvXAddress & address) {
    AddressToPositionMap::iterator it = addressToPositionMap.find(address);
    addressToPositionMap.erase(it);
}

void VS_PositionTable::removeOldPositions(simtime_t timestamp) {
...
void VS_PositionTable::clear() {
    addressToPositionMap.clear();
}

simtime_t VS_PositionTable::getOldestPosition() const {
