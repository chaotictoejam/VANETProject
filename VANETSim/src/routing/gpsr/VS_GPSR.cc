...
Define_Module(VS_GPSR);

#define VS_GPSR_EV EV << "GPSR at " << getHostName() << " "
...
//VS_PositionTable VS_GPSR::globalPositionTable;
...
void VS_GPSR::initialize(int stage)
{
    ...
    {
        // VS_GPSR parameters
        planarizationMode = (VS_GPSRPlanarizationMode)(int)par("planarizationMode");
...
void VS_GPSR::handleMessage(cMessage * message)
{
...
void VS_GPSR::processSelfMessage(cMessage * message)
{
...
void VS_GPSR::processMessage(cMessage * message)
{
...
void VS_GPSR::scheduleBeaconTimer()
{
    VS_GPSR_EV << "Scheduling beacon timer" << endl;
    scheduleAt(simTime() + beaconInterval, beaconTimer);
}

void VS_GPSR::processBeaconTimer()
{
    VS_GPSR_EV << "Processing beacon timer" << endl;
...
void VS_GPSR::schedulePurgeNeighborsTimer()
{
    VS_GPSR_EV << "Scheduling purge neighbors timer" << endl;
...
void VS_GPSR::processPurgeNeighborsTimer()
{
    VS_GPSR_EV << "Processing purge neighbors timer" << endl;
    purgeNeighbors();
    schedulePurgeNeighborsTimer();
}
...
void VS_GPSR::sendUDPPacket(UDPPacket * packet, double delay)
{
    if (delay == 0)
        send(packet, "ipOut");
    else
...
void VS_GPSR::processUDPPacket(UDPPacket * packet)
{
    cPacket * encapsulatedPacket = packet->decapsulate();
    if (dynamic_cast<VS_GPSRBeacon *>(encapsulatedPacket))
        processBeacon((VS_GPSRBeacon *)encapsulatedPacket);
    else
...
VS_GPSRBeacon * VS_GPSR::createBeacon()
{
    VS_GPSRBeacon * beacon = new VS_GPSRBeacon();
    beacon->setAddress(getSelfAddress());
    beacon->setPosition(mobility->getCurrentPosition());
    return beacon;
}

void VS_GPSR::sendBeacon(VS_GPSRBeacon * beacon, double delay)
{
    VS_GPSR_EV << "Sending beacon: address = " << beacon->getAddress() << ", position = " << beacon->getPosition() << endl;
    ...
    udpPacket->setSourcePort(VS_GPSR_UDP_PORT);
    udpPacket->setDestinationPort(VS_GPSR_UDP_PORT);
    udpPacket->setControlInfo(networkProtocolControlInfo);
    sendUDPPacket(udpPacket, delay);
}

void VS_GPSR::processBeacon(VS_GPSRBeacon * beacon)
{
    VS_GPSR_EV << "Processing beacon: address = " << beacon->getAddress() << ", position = " << beacon->getPosition() << endl;
...
VS_GPSRPacket * VS_GPSR::createPacket(IPvXAddress destination, cPacket * content)
{
    VS_GPSRPacket * gpsrPacket = new VS_GPSRPacket(content->getName());
    gpsrPacket->setRoutingMode(VS_GPSR_GREEDY_ROUTING);
    // KLUDGE: implement position registry protocol
...
int VS_GPSR::computePacketBitLength(VS_GPSRPacket * packet)
{
    // routingMode
    int routingMode = 1;
...
bool VS_GPSR::isNodeUp() const
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

void VS_GPSR::configureInterfaces()
{
...
Coord VS_GPSR::intersectSections(Coord & begin1, Coord & end1, Coord & begin2, Coord & end2)
{
    double x1 = begin1.x;
    double y1 = begin1.y;
    double x2 = end1.x;
...
Coord VS_GPSR::getDestinationPosition(const IPvXAddress & address) const
{
    // KLUDGE: implement position registry protocol
    return globalPositionTable.getPosition(address);
}

Coord VS_GPSR::getNeighborPosition(const IPvXAddress & address) const
{
...
double VS_GPSR::getVectorAngle(Coord vector)
{
...
double VS_GPSR::getDestinationAngle(const IPvXAddress & address)
{
    return getVectorAngle(getDestinationPosition(address) - mobility->getCurrentPosition());
}

double VS_GPSR::getNeighborAngle(const IPvXAddress & address)
{
...
std::string VS_GPSR::getHostName() const
{
    return host->getFullName();
}

IPvXAddress VS_GPSR::getSelfAddress() const
{
    return routingTable->getRouterId();
}

IPvXAddress VS_GPSR::getSenderNeighborAddress(IPv4Datagram * datagram) const
{
    VS_GPSRPacket * packet = check_and_cast<VS_GPSRPacket *>(dynamic_cast<cPacket *>(datagram)->getEncapsulatedPacket());
...
simtime_t VS_GPSR::getNextNeighborExpiration()
{
...
void VS_GPSR::purgeNeighbors()
{
    neighborPositionTable.removeOldPositions(simTime() - neighborValidityInterval);
}

std::vector<IPvXAddress> VS_GPSR::getPlanarNeighbors()
{
    ...
    for (std::vector<IPvXAddress>::iterator it = neighborAddresses.begin(); it != neighborAddresses.end(); it++) {
        ...
        if (planarizationMode == VS_GPSR_RNG_PLANARIZATION) {
        ...
        else if (planarizationMode == VS_GPSR_GG_PLANARIZATION) {
...
IPvXAddress VS_GPSR::getNextPlanarNeighborCounterClockwise(const IPvXAddress& startNeighborAddress, double startNeighborAngle)
{
    VS_GPSR_EV << "Finding next planar neighbor (counter clockwise): startAddress = " << startNeighborAddress << ", startAngle = " << startNeighborAngle << endl;
    ...
    for (std::vector<IPvXAddress>::iterator it = neighborAddresses.begin(); it != neighborAddresses.end(); it++) {
        ...
        VS_GPSR_EV << "Trying next planar neighbor (counter clockwise): address = " << neighborAddress << ", angle = " << neighborAngle << endl;
...
IPvXAddress VS_GPSR::findNextHop(IPv4Datagram * datagram, const IPvXAddress & destination)
{
    VS_GPSRPacket * packet = check_and_cast<VS_GPSRPacket *>(dynamic_cast<cPacket *>(datagram)->getEncapsulatedPacket());
    if (packet->getRoutingMode() == VS_GPSR_GREEDY_ROUTING)
        return findGreedyRoutingNextHop(datagram, destination);
    else if (packet->getRoutingMode() == VS_GPSR_PERIMETER_ROUTING)
        return findPerimeterRoutingNextHop(datagram, destination);
    else
        throw cRuntimeError("Unknown routing mode");
}

IPvXAddress VS_GPSR::findGreedyRoutingNextHop(IPv4Datagram * datagram, const IPvXAddress & destination)
{
    VS_GPSR_EV << "Finding next hop using greedy routing: destination = " << destination << endl;
    VS_GPSRPacket * packet = check_and_cast<VS_GPSRPacket *>(dynamic_cast<cPacket *>(datagram)->getEncapsulatedPacket());
    IPvXAddress selfAddress = getSelfAddress();
    Coord selfPosition = mobility->getCurrentPosition();
    ...
    if (bestNeighbor.isUnspecified()) {
        VS_GPSR_EV << "Switching to perimeter routing: destination = " << destination << endl;
        packet->setRoutingMode(VS_GPSR_PERIMETER_ROUTING);
...
IPvXAddress VS_GPSR::findPerimeterRoutingNextHop(IPv4Datagram * datagram, const IPvXAddress & destination)
{
    VS_GPSR_EV << "Finding next hop using perimeter routing: destination = " << destination << endl;
    VS_GPSRPacket * packet = check_and_cast<VS_GPSRPacket *>(dynamic_cast<cPacket *>(datagram)->getEncapsulatedPacket());
    IPvXAddress selfAddress = getSelfAddress();
    Coord selfPosition = mobility->getCurrentPosition();
    ...
    if (selfDistance < perimeterRoutingStartDistance) {
        VS_GPSR_EV << "Switching to greedy routing: destination = " << destination << endl;
        packet->setRoutingMode(VS_GPSR_GREEDY_ROUTING);
        ...
        do {
            ...
            if (nextNeighborAddress.isUnspecified())
                break;
            VS_GPSR_EV << "Intersecting towards next hop: nextNeighbor = " << nextNeighborAddress << ", firstSender = " << firstSenderAddress << ", firstReceiver = " << firstReceiverAddress << ", destination = " << destination << endl;
            ...
            if (hasIntersection) {
                VS_GPSR_EV << "Edge to next hop intersects: intersection = " << intersection << ", nextNeighbor = " << nextNeighborAddress << ", firstSender = " << firstSenderAddress << ", firstReceiver = " << firstReceiverAddress << ", destination = " << destination << endl;
                packet->setCurrentFaceFirstSenderAddress(selfAddress);
                packet->setCurrentFaceFirstReceiverAddress(IPvXAddress());
            }
        }
        ...
        if (firstSenderAddress == selfAddress && firstReceiverAddress == nextNeighborAddress) {
            VS_GPSR_EV << "End of perimeter reached: firstSender = " << firstSenderAddress << ", firstReceiver = " << firstReceiverAddress << ", destination = " << destination << endl;
            return IPvXAddress();
        }
        else {
...
INetfilter::IHook::Result VS_GPSR::routeDatagram(IPv4Datagram * datagram, const InterfaceEntry *& outputInterfaceEntry, IPv4Address & nextHop)
{
    const IPvXAddress source = datagram->getSrcAddress();
    const IPvXAddress destination = datagram->getDestAddress();
    VS_GPSR_EV << "Finding next hop: source = " << source << ", destination = " << destination << endl;
    nextHop = findNextHop(datagram, destination).get4();
    if (nextHop.isUnspecified()) {
        VS_GPSR_EV << "No next hop found, dropping packet: source = " << source << ", destination = " << destination << endl;
        return DROP;
    }
    else {
        VS_GPSR_EV << "Next hop found: source = " << source << ", destination = " << destination << ", nextHop: " << nextHop << endl;
        VS_GPSRPacket * packet = check_and_cast<VS_GPSRPacket *>(dynamic_cast<cPacket *>(datagram)->getEncapsulatedPacket());
...
INetfilter::IHook::Result VS_GPSR::datagramPreRoutingHook(IPv4Datagram * datagram, const InterfaceEntry * inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, IPv4Address & nextHop)
{
...
INetfilter::IHook::Result VS_GPSR::datagramLocalInHook(IPv4Datagram * datagram, const InterfaceEntry * inputInterfaceEntry)
{
    cPacket * networkPacket = dynamic_cast<cPacket *>(datagram);
    VS_GPSRPacket * gpsrPacket = dynamic_cast<VS_GPSRPacket *>(networkPacket->getEncapsulatedPacket());
    if (gpsrPacket) {
...
INetfilter::IHook::Result VS_GPSR::datagramLocalOutHook(IPv4Datagram * datagram, const InterfaceEntry *& outputInterfaceEntry, IPv4Address & nextHop)
{
    const IPv4Address & destination = datagram->getDestAddress();
    if (destination.isMulticast() || destination.isLimitedBroadcastAddress() || routingTable->isLocalAddress(destination))
        return ACCEPT;
        ...
        cPacket * networkPacket = dynamic_cast<cPacket *>(datagram);
        VS_GPSRPacket * gpsrPacket = createPacket(datagram->getDestAddress(), networkPacket->decapsulate());
...
bool VS_GPSR::handleOperationStage(LifecycleOperation * operation, int stage, IDoneCallback * doneCallback)
{
    Enter_Method_Silent();
...
void VS_GPSR::receiveChangeNotification(int signalID, const cObject *obj)
{
    Enter_Method("receiveChangeNotification");
    if (signalID == NF_LINK_BREAK) {
        VS_GPSR_EV << "Received link break" << endl;
