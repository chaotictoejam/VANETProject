...
Register_Class(VS_AODV_msg);


VS_AODV_msg::VS_AODV_msg(const VS_AODV_msg& m) : cPacket(m)
{
    copy(m);
}


VS_AODV_msg& VS_AODV_msg::operator=(const VS_AODV_msg& m)
{
...
void VS_AODV_msg::copy(const VS_AODV_msg& m)
{
    type = m.type;
    ttl = m.ttl;
    prevFix = m.prevFix;
    ...
    extension = new VS_AODV_ext [extensionsize];
...
void VS_AODV_msg:: clearExtension()
{
    if (extensionsize==0)
    {
        return;
...
VS_AODV_ext * VS_AODV_msg::addExtension(int type, int len, char *data)
{
    VS_AODV_ext* extension_aux;
    if (len<0)
    {
        return NULL;
    }
    extension_aux =   new VS_AODV_ext [extensionsize+1];
    for (int i=0; i < extensionsize; i++)
    {
    ...
    setBitLength(getBitLength()+((VS_AODV_EXT_HDR_SIZE+len) *8));
    return & extension[extensionsize-1];
}


VS_AODV_ext * VS_AODV_msg::getNexExtension(VS_AODV_ext* aodv_ext)
{
...
RERR::RERR(const RERR& m) : VS_AODV_msg(m)
{
    copy(m);
}
...
Register_Class(RREP);
RREP::RREP(const RREP& m) : VS_AODV_msg(m)
{
    copy(m);
}
...
RREP_ack::RREP_ack(const RREP_ack& m) : VS_AODV_msg(m)
{
    copy(m);
}
...
Register_Class(RREQ);
RREQ::RREQ(const RREQ& m) : VS_AODV_msg(m)
{
    copy(m);
}
