...
EXECUTE_ON_STARTUP(
    cEnum *e = cEnum::find("BGPPathSegmentType");
    if (!e) enums.getInstance()->add(e = new cEnum("BGPPathSegmentType"));
...
BGPASPathSegmentDescriptor::BGPASPathSegmentDescriptor() : cClassDescriptor("BGPASPathSegment", "cObject")
{
}
...
const char *BGPASPathSegmentDescriptor::getFieldProperty(void *object, int field, const char *propertyname) const
{
    ...
    switch (field) {
        case 0:
            if (!strcmp(propertyname,"enum")) return "BGPPathSegmentType";
            return NULL;
        default: return NULL;
    }
}
